# Host Monitoring Server 0.9.8 发行包部署手册

本文只面向已经生成的正式 Server 发行物：

```text
host-monitoring-server-0.9.8-x86_64-unknown-linux-gnu.tar.gz
host-monitoring-server-0.9.8-x86_64-unknown-linux-gnu.tar.gz.sha256
```
...
- 产品内没有升级、迁移、备份或恢复命令。未来只有 `sarmg-upgrade` 为精确输入/输出版本声明并验证
  转换边后，才存在受支持的数据操作。
- 正式发行目录固定为 `/opt/isarmg/host-monitoring/releases/0.9.8`。禁止创建 `current`、`latest`
  等可变别名。
...
sha256sum --check --strict \
  host-monitoring-server-0.9.8-x86_64-unknown-linux-gnu.tar.gz.sha256
tar -tzf host-monitoring-server-0.9.8-x86_64-unknown-linux-gnu.tar.gz
```

预安装验证必须把 `0.9.8` 解压到名为 `releases` 的真实父目录，因为这是发行 root 合同的一部分：

```bash
temporary_root="$(mktemp -d)"
mkdir -m 0755 "$temporary_root/releases"
tar --extract --gzip \
  --file host-monitoring-server-0.9.8-x86_64-unknown-linux-gnu.tar.gz \
  --directory "$temporary_root/releases" \
  --no-same-owner --same-permissions --delay-directory-restore

"$temporary_root/releases/0.9.8/bin/host-monitoring-server" identity
"$temporary_root/releases/0.9.8/bin/host-monitoring-server" verify-release \
  --root "$temporary_root/releases/0.9.8"
```
...
```bash
sudo test ! -e /opt/isarmg/host-monitoring/releases/0.9.8
...
sudo chown -R root:root /opt/isarmg/host-monitoring/releases/0.9.8

sudo /opt/isarmg/host-monitoring/releases/0.9.8/bin/host-monitoring-server \
  verify-release --root /opt/isarmg/host-monitoring/releases/0.9.8
```
...
`/opt/isarmg/host-monitoring/releases/0.9.8/web`；不得指向源码、旧 dist、软链接或在线编辑目录。
...
sudo install -m 0644 -o root -g root \
  /opt/isarmg/host-monitoring/releases/0.9.8/systemd/host-monitoring-server.service \
...
- 固定 `serve-release --root /opt/isarmg/host-monitoring/releases/0.9.8`；
...
### `release root must be releases/0.9.8`

root 的直接父目录必须名为 `releases`，root 必须名为 `0.9.8`，整条路径必须规范化且不能经过 symlink。
...
- [ ] 归档只有 `0.9.8` 顶层目录，identity 和 `verify-release` 通过。
...
全部通过后，才表示当前 `0.9.8` Server 发行具备上线条件。
