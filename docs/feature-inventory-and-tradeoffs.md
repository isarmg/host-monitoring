...
| HOST-035 | Server 共享原语固定 Foundation `=0.7.0` + 完整 revision `77e7ad7af8e1bf62432bd6bdd8fa9aff54cb39d1`；八个 Web 包固定同版 Release URL + lock integrity；Agent 使用独立上游 | 根 `Cargo.toml`、`clients/web/{package.json,package-lock.json}`、Foundation 门禁 | 保障 | 高 | 认证、构建及 SQLite 基线漂移；sibling/path 会破坏独立 checkout | Foundation 合同、完整 rev/八个 URL/integrity、Router→Agent、Web clean build、SQLite reopen |
