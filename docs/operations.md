...
```text
/opt/isarmg/host-monitoring/releases/0.9.7/   root 持有、只读发行树
...
```text
ExecStart=/opt/isarmg/host-monitoring/releases/0.9.7/bin/host-monitoring-server \
  serve-release --root /opt/isarmg/host-monitoring/releases/0.9.7
```
...
在 x86_64 glibc Linux 上，从干净、annotated `v0.9.7` 精确指向 HEAD 的 checkout，向仓库外已存在目录
构建：
...
当前 Server Rust 固定 Foundation 0.7.3 / `45006e4b981666529f4e5003bbf6a76cb60578ff`，八个 Web 包使用
...
host-monitoring-server verify-release --root /opt/isarmg/host-monitoring/releases/0.9.7
...
```powershell
clients\host-monitor\packaging\windows\wix\build-msi.cmd 0.9.7 `
...
Server 只创建当前库。`product_metadata` 必须精确绑定 application `host-monitoring`、version `0.9.7`、
