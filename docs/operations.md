...
```text
ExecStart=/opt/isarmg/host-monitoring/releases/0.9.3/bin/host-monitoring-server \
  serve-release --root /opt/isarmg/host-monitoring/releases/0.9.3
```
...
在 x86_64 glibc Linux 上，从干净、annotated `v0.9.3` 精确指向 HEAD 的 checkout，向仓库外已存在目录
构建：
...
host-monitoring-server verify-release --root /opt/isarmg/host-monitoring/releases/0.9.3
...
```powershell
clients\host-monitor\packaging\windows\wix\build-msi.cmd 0.9.3 `
...
Server 只创建当前库。`product_metadata` 必须精确绑定 application `host-monitoring`、version `0.9.3`、
