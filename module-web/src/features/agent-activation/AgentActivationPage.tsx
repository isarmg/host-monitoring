      ...
      <section className="activation-screen">
        <section className="activation-card" aria-labelledby="agent-activation-title">
          <div className="activation-heading success">
            ...
            <div>
              <h1 id="agent-activation-title">host-m-agent 激活成功</h1>
    ...
    <section className="activation-screen">
      <section className="activation-card" aria-labelledby="agent-activation-title">
        <div className="activation-heading">
          ...
          <div>
            <h1 id="agent-activation-title">激活 host-m-agent</h1>
            <p>CLI 配对会在此页面确认一次性授权密钥；Windows 可直接在 host-m-agent 本地配置页填写服务器地址和授权密钥。</p>
          </div>
        </div>

        {requestId && pairingQuery.data ? (
          <dl className="activation-summary" aria-label="host-m-agent 配对摘要">
            <div>
              <dt>系统</dt>
        ...
        {!requestId ? (
          <div className="activation-route-error" role="alert">
            激活链接无效或不完整。请返回 host-m-agent 重新发起浏览器配对。
          </div>
        ) : null}

        {requestId && pairingQuery.isLoading ? <LoadingBlock label="正在读取 host-m-agent 配对信息" /> : null}
