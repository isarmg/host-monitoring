...
- 移除 Union 模块契约，改为独立 `HOST_MONITORING_*` 环境变量和本地管理员会话；
  0.7 使用独立数据库和本地状态，并重新配对 Agent。
- Host Server 只初始化 0.7 的唯一当前 SQLite Schema，并用 `product_metadata` 与
  `sqlite_schema` SHA-256 双重校验精确格式；迁移、升级、备份和恢复从产品二进制移除。
...
- Host Server 保持独立 SQLite 数据边界、loopback-only 本地服务和本地管理端入口。
