...
mod tests {
    ...
    fn backup_is_verified_non_overwriting_and_restores_atomically() {
        ...
        let output = directory.path().join("backup.sqlite3");
        create_product_database(&database);
        let database_url = format!("sqlite://{}", database.display());
        create(&database_url, &output).unwrap();
        verify(&output).unwrap();
        assert!(create(&database_url, &output).is_err());
        ...
        connection
            ...
            .unwrap();
        drop(connection);
        restore(&database_url, &output).unwrap();
    ...
    #[test]
    fn database_url_parser_rejects_memory_and_ambiguous_urls() {
