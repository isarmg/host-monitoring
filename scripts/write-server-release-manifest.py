...
def read_identity(binary: Path) -> tuple[dict[str, object], bytes]:
    ...
    if (
        ...
        fail("release binary is not the exact bound Host Monitoring 0.7 identity")
    return identity, encoded


def main() -> None:
    if len(sys.argv) != 2:
        fail("usage: write-server-release-manifest.py /absolute/releases/0.9.0")
    root = Path(sys.argv[1])
    if not root.is_absolute() or root.name != VERSION or root.parent.name != "releases":
        fail("root must be an absolute releases/0.9.0 directory")
