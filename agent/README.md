...
`unionc-agent` 是 Union Host Monitoring 的跨平台主机侧组件。Linux、Windows 和 macOS 使用
...
cargo check --locked -p unionc-agent --target aarch64-linux-android --lib --no-default-features
cargo check --locked -p unionc-agent --target aarch64-apple-ios --lib --no-default-features
cargo check --locked -p unionc-agent --target aarch64-apple-ios-sim --lib --no-default-features
```
