...
mkdir -p \
  "$fixture_root/clients/host-monitor/packaging/linux" \
  "$fixture_root/config" \
  "$fixture_root/target/release" \
  "$mock_bin"
cp "$packaging_dir/build-packages.sh" \
  "$fixture_root/clients/host-monitor/packaging/linux/build-packages.sh"
cp "$packaging_dir/../nfpm.yaml" "$fixture_root/clients/host-monitor/packaging/nfpm.yaml"
for lifecycle_script in postinstall.sh preremove.sh postremove.sh purge-local-state.sh; do
  printf 'package_version=%s\n' "$package_version" \
    >"$fixture_root/clients/host-monitor/packaging/linux/$lifecycle_script"
done
...
printf '%s\n' 'path+file:///fixture/agent#host-monitor@$package_version'
EOF
...
run_builder() {
  ...
  (
    ...
    sh clients/host-monitor/packaging/linux/build-packages.sh
  ) >"$builder_output" 2>&1
}
...
do
  ...
  printf 'package_version=0.3.3\n' \
    >"$fixture_root/clients/host-monitor/packaging/linux/$stale_lifecycle_script"
  if run_builder amd64; then
    fail "builder accepted stale $stale_lifecycle_script package_version"
  fi
  assert_output_contains \
    "clients/host-monitor/packaging/linux/$stale_lifecycle_script package_version does not match Cargo"
