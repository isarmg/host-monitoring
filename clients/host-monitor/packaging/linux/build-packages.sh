...
package_id=$(cargo pkgid --locked -p host-monitor) || die "cannot resolve the Agent Cargo package"
case "$package_id" in
...
for lifecycle_script in \
  clients/host-monitor/packaging/linux/postinstall.sh \
  clients/host-monitor/packaging/linux/preremove.sh \
  clients/host-monitor/packaging/linux/postremove.sh \
  clients/host-monitor/packaging/linux/purge-local-state.sh
do
...
agent_binary=target/release/host-monitor
...
LC_ALL=C readelf -p .host_monitor.version -- "$agent_binary" 2>/dev/null |
...
[ "$config_version" = "$package_version" ] ||
  die "host-monitor.json.example version $config_version does not match Agent $package_version"
...
VERSION="$package_version" NFPM_ARCH="$package_arch" "$nfpm_bin" package \
  --config clients/host-monitor/packaging/nfpm.yaml --packager deb \
  --target "dist/host-monitor_${package_version}_${package_arch}.deb"
VERSION="$package_version" NFPM_ARCH="$package_arch" "$nfpm_bin" package \
  --config clients/host-monitor/packaging/nfpm.yaml --packager rpm \
