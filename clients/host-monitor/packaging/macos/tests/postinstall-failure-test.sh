...
make_case() {
  ...
  case_agent_binary="$case_dir/usr/local/libexec/host-monitor"
  case_agent_command="$case_dir/usr/local/bin/host-monitor"
  case_logrotate_helper="$case_dir/usr/local/libexec/host-monitor-logrotate"
  case_agent_plist="$case_dir/Library/LaunchDaemons/org.sarmg.hostmonitor.plist"
...
EOF
  ...
  ln -s ../libexec/host-monitor "$case_agent_command"
...
case "$path" in
  ...
  "$CASE_ROOT/Library/Application Support")
    ...
    permissions=drwxr-xr-x
    ;;
  "$CASE_ROOT/usr/local/libexec/host-monitor") acl_key=agent-binary ;;
  "$CASE_ROOT/usr/local/bin/host-monitor")
    acl_key=agent-command
    permissions=lrwxr-xr-x
    ;;
  "$CASE_ROOT/usr/local/libexec/host-monitor-logrotate") acl_key=logrotate-helper ;;
  "$CASE_ROOT/Library/LaunchDaemons/org.sarmg.hostmonitor.plist") acl_key=agent-plist ;;
  "$CASE_ROOT/Library/LaunchDaemons/org.sarmg.hostmonitor.logrotate.plist")
  ...
  case "$payload_failure" in
    ...
    agent-version)
      sed 's/host-monitor @HOST_MONITOR_PACKAGE_VERSION@/host-monitor 0.0.0/' \
        "$case_dir/usr/local/libexec/host-monitor" >"$case_dir/agent.invalid"
      mv "$case_dir/agent.invalid" "$case_dir/usr/local/libexec/host-monitor"
