...
impl Fixture {
    fn new() -> Self {
        let root = std::env::temp_dir().join(format!(
            "host-m-agent-pairing-transport-policy-{}",
            Uuid::new_v4()
        ));
        ...
        config.endpoint = "http://192.0.2.10:1/api/v2/agent/report".into();
        config.pairing_endpoint = Some("https://192.0.2.10:1/api/v2/agent/pairing-requests".into());
    ...
    fn pair(&self, cli_override: bool, environment_override: bool) -> std::process::Output {
        let mut command = Command::new(env!("CARGO_BIN_EXE_host-m-agent"));
        command.args(["pair", "--config", self.config_path.to_str().unwrap()]);
        if cli_override {
            command.arg("--allow-insecure-http");
        }
        command.env_remove("HOST_M_AGENT_ALLOW_INSECURE_HTTP");
        if environment_override {
            command.env("HOST_M_AGENT_ALLOW_INSECURE_HTTP", "true");
        }
