...
`config.json`；它只用于让尚未配对的服务健康常驻，随后由 `pair` 原子更新。同一 0.7.0
包的重装和普通卸载都不覆盖已有配置。
...
```cmd
msiexec.exe /i host-m-agent-0.7.0-x64.msi /qn /norestart /l*v "%TEMP%\host-m-agent-install.log"
```
...
安装前原生维护程序只检查当前 0.7.0 SCM 服务、固定程序路径和当前精确 ACL。任何同名但
...
- 本轮开始时目录不存在；
- 存在当前 0.7.0 创建的、版本绑定的 managed-state marker，且其内容、owner、精确 DACL、
...
```cmd
msiexec.exe /x host-m-agent-0.7.0-x64.msi /qn /norestart /l*v "%TEMP%\host-m-agent-uninstall.log"
```
...
隔离名称 `%ProgramData%\host-m-agent.purge-quarantine-0.7.0`。在事务进入 commit 之前，任一
...
清理，可先重新安装可信的 0.7.0 MSI（版本 marker 与精确 ACL 联合验证后接管），再执行带
`PURGE=1` 的卸载。
...
host-m-agent\packaging\windows\wix\build-msi.cmd 0.7.0 ^
...
其输出精确等于 `host-m-agent 0.7.0`；不能用其他版本号包装当前二进制。
...
.\host-m-agent\packaging\windows\tests\Test-MsiLifecycle.ps1 `
  -ProductVersion 0.7.0 -ArtifactDirectory .\dist
```
