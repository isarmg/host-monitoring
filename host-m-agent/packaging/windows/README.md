...
```cmd
msiexec.exe /i host-m-agent-0.5.0-x64.msi /qn /norestart /l*v "%TEMP%\host-m-agent-install.log"
```
...
```cmd
msiexec.exe /x host-m-agent-0.5.0-x64.msi /qn /norestart /l*v "%TEMP%\host-m-agent-uninstall.log"
```
...
```cmd
msiexec.exe /x host-m-agent-0.5.0-x64.msi PURGE=1 /qn /norestart /l*v "%TEMP%\host-m-agent-purge.log"
```

Purge 先完整扫描状态树并拒绝所有 reparse point，然后在同一卷原子移动到受保护的固定
隔离名称 `%ProgramData%\host-m-agent.purge-quarantine-0.5.0`。在事务进入 commit 之前，任一
...
host-m-agent\packaging\windows\wix\build-msi.cmd 0.5.0 ^
...
其输出精确等于 `host-m-agent 0.5.0`；不能用其他版本号包装当前二进制。
