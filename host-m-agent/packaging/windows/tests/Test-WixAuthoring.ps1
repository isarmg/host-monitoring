...
$projectPath = Join-Path $wixRoot "HostMAgent.Installer.wixproj"
...
$helperPath = Join-Path $agentRoot "src\bin\host-m-agent-maintenance.rs"
$trayPath = Join-Path $agentRoot "src\bin\host-m-agent-tray.rs"
...
Assert-Equal $service.Arguments `
    '--windows-service run --config "[CommonAppDataFolder]host-m-agent\config.json"' `
    "The SCM entrypoint and fixed config path drifted."

$serviceControl = Select-One "//w:ServiceControl[@Name='host-m-agent']"
...
Assert-Equal $trayFile.Name "host-m-agent-tray.exe" "Unexpected installed tray filename."
...
$trayRun = Select-One `
    "//w:Component[@Id='AgentTrayComponent']/w:RegistryValue[@Name='host-m-agent-tray']"
...
Assert-Equal $trayRun.Value '"[INSTALLFOLDER]host-m-agent-tray.exe" --startup' `
...
Assert-Equal $closeTray.Target "host-m-agent-tray.exe" `
...
foreach ($entry in $expectedActions.GetEnumerator()) {
    ...
    Assert-Equal $action.BinaryRef "HostMAgentMaintenance.exe" `
...
Assert-Equal $defaultProductVersions[0].InnerText `
    ...
    "The default WiX ProductVersion must match the host-m-agent workspace package version."
...
Assert-Contains $projectText "'`$(DetectedAgentVersion)' != 'host-m-agent `$(ProductVersion)'" `
...
Assert-Contains $buildText 'host-m-agent %PRODUCT_VERSION%' `
