...
if ([string]::IsNullOrWhiteSpace($LogDirectory)) {
    ...
    $LogDirectory = Join-Path $temporaryRoot "host-m-agent-msi-logs"
}
...
$installedRoot = Join-Path $env:ProgramFiles "host-m-agent"
$installedTray = Join-Path $installedRoot "host-m-agent-tray.exe"
$stateRoot = Join-Path $env:ProgramData "host-m-agent"
$trayRunKey = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
$trayRunName = "host-m-agent-tray"
$commonPrograms = [Environment]::GetFolderPath(
    [Environment+SpecialFolder]::CommonPrograms
)
$trayShortcut = Join-Path $commonPrograms "host-m-agent.lnk"
$stateMarker = ".host-m-agent-managed-$ProductVersion"
$installJournal = Join-Path $env:ProgramData `
    "host-m-agent.install-journal-$ProductVersion"
$uninstallJournal = Join-Path $env:ProgramData `
    "host-m-agent.uninstall-journal-$ProductVersion"
$purgeQuarantine = Join-Path $env:ProgramData `
    "host-m-agent.purge-quarantine-$ProductVersion"
$maintenanceDiagnostic = Join-Path $env:ProgramData `
    "host-m-agent.maintenance-diagnostic-$ProductVersion.txt"
...
function Read-AndRemoveMaintenanceDiagnostic {
    ...
    if ($fields.Count -ne 4 -or
        $fields[0] -cne "format=host-m-agent-maintenance-diagnostic-v1" -or
...
function Assert-ServiceRunning {
    $service = Get-Service -Name "host-m-agent" -ErrorAction Stop
    $service.WaitForStatus([System.ServiceProcess.ServiceControllerStatus]::Running,
        [TimeSpan]::FromSeconds(30))
    $definition = Get-CimInstance Win32_Service -Filter "Name='host-m-agent'"
    if ($definition.StartMode -ne "Auto" -or
        ...
        throw "Installed SCM service definition is not the expected host-m-agent service."
    }
    $serviceKey = "HKLM:\SYSTEM\CurrentControlSet\Services\host-m-agent"
    if ((Get-ItemPropertyValue -LiteralPath $serviceKey -Name ServiceSidType) -ne 1) {
        throw "host-m-agent does not have an unrestricted service SID."
    }
    if ((Get-ItemPropertyValue -LiteralPath $serviceKey `
        -Name FailureActionsOnNonCrashFailures) -ne 1) {
        throw "host-m-agent does not enable failure actions for non-crash failures."
    }
}

function Assert-StateAcl {
    $serviceSid = (New-Object System.Security.Principal.NTAccount(
        "NT SERVICE", "host-m-agent"
...
function Assert-TrayIntegration {
    if (Get-Process -Name "host-m-agent-tray" -ErrorAction SilentlyContinue) {
    ...
    if (-not (Test-Path -LiteralPath $trayShortcut -PathType Leaf)) {
        throw "The host-m-agent Start menu shortcut is missing."
    }
...
function Get-HostMAgentArpEntries {
    ...
    return @(Get-ChildItem -LiteralPath $uninstallRoot | Get-ItemProperty |
        Where-Object {
            ...
            $null -ne $displayName -and $displayName.Value -eq "host-m-agent"
        })
}

function Assert-ArpVersion([string]$ExpectedVersion) {
    $entries = @(Get-HostMAgentArpEntries)
    if ($entries.Count -ne 1 -or $entries[0].DisplayVersion -ne $ExpectedVersion) {
        throw "Apps & Features does not contain exactly host-m-agent $ExpectedVersion."
    }
}
...
function Assert-AgentCompletelyAbsent {
    if ((Test-Path -LiteralPath $installedRoot) -or
        ...
        (Get-Service -Name "host-m-agent" -ErrorAction SilentlyContinue) -or
        (Get-Process -Name "host-m-agent-tray" -ErrorAction SilentlyContinue)) {
        throw "Agent installation or a protected transaction artifact survived purge."
    }
    $entries = @(Get-HostMAgentArpEntries)
    if ($entries.Count -ne 0) {
        throw "Apps & Features still contains host-m-agent."
    }
}
...
$existingArpEntries = @(Get-HostMAgentArpEntries)
$runningTrayProcesses = @(Get-Process -Name "host-m-agent-tray" `
...
if ((Test-Path -LiteralPath $installedRoot) -or
    ...
    throw ("The disposable runner contains an existing host-m-agent installation, " +
...
New-Service -Name "host-m-agent" -BinaryPathName "$env:SystemRoot\System32\cmd.exe /c exit 0" `
    -StartupType Manual | Out-Null
$foreignPath = (Get-CimInstance Win32_Service -Filter "Name='host-m-agent'").PathName
Invoke-Msi /i $currentMsi "reject-foreign-service" -ExpectFailure
$remainingForeignPath = (Get-CimInstance Win32_Service -Filter "Name='host-m-agent'").PathName
if ($remainingForeignPath -ne $foreignPath) {
    throw "Rejected install modified the foreign SCM service."
}
& sc.exe delete host-m-agent | Out-Host
if ($LASTEXITCODE -ne 0) { throw "Could not remove the smoke-test foreign service." }
for ($attempt = 0; $attempt -lt 30; $attempt++) {
    if (-not (Get-Service -Name "host-m-agent" -ErrorAction SilentlyContinue)) { break }
    Start-Sleep -Milliseconds 500
}
if (Get-Service -Name "host-m-agent" -ErrorAction SilentlyContinue) {
...
if (@(Get-HostMAgentArpEntries).Count -ne 0) {
    throw "Apps & Features still contains host-m-agent after ordinary uninstall."
}
