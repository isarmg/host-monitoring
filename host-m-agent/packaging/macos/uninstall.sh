...
identifier="org.sarmg.hostmagent"
user="_hostmagent"
group="_hostmagent"
state="/Library/Application Support/host-m-agent"
log="/var/log/host-m-agent.log"
share="/usr/local/share/host-m-agent"
ownership_dir="/var/db/host-m-agent"
ownership_marker="$ownership_dir/account-ownership"
package_version="@HOST_M_AGENT_PACKAGE_VERSION@"
purge=0
assume_yes=0
user_created=0
group_created=0
...
stop_job system/org.sarmg.hostmagent.logrotate
stop_job system/org.sarmg.hostmagent

rm -f /Library/LaunchDaemons/org.sarmg.hostmagent.logrotate.plist
rm -f /Library/LaunchDaemons/org.sarmg.hostmagent.plist
rm -f /usr/local/libexec/host-m-agent-logrotate
rm -f /usr/local/libexec/host-m-agent
rm -f "$share/newsyslog.conf"
rm -f "$share/config.example.json"

command_link="/usr/local/bin/host-m-agent"
if [ -L "$command_link" ]; then
  link_target="$(readlink "$command_link")"
  case "$link_target" in
    ../libexec/host-m-agent|/usr/local/libexec/host-m-agent)
      rm -f "$command_link"
      ;;
    *)
...
if [ "$purge" -ne 1 ]; then
  ...
  cat <<EOF
host-m-agent program and LaunchDaemons were removed.
...
rm -rf "$state"
rm -f "$log"
for archived_log in /var/log/host-m-agent.log.*; do
...
if [ "$purge_incomplete" -eq 1 ]; then
  cat >&2 <<EOF
host-m-agent purge is incomplete: local state, logs, and program files were removed, but an
...
echo "host-m-agent program and installer-owned local data were purged; server revoke is separate."
exit 0
