...
output="${OUTPUT:-host-m-agent-$VERSION.pkg}"
...
[ "$binary_version" = "host-m-agent $VERSION" ] ||
...
plutil -lint "$script_dir/org.sarmg.hostmagent.plist" >/dev/null
plutil -lint "$script_dir/org.sarmg.hostmagent.logrotate.plist" >/dev/null
...
for installer_script in preinstall postinstall; do
  sed "s/@HOST_M_AGENT_PACKAGE_VERSION@/$VERSION/g" \
...
install -d "$root/usr/local/libexec" "$root/usr/local/bin" \
  "$root/usr/local/share/host-m-agent" "$root/Library/LaunchDaemons"
install -m 0755 "$BINARY" "$root/usr/local/libexec/host-m-agent"
ln -s ../libexec/host-m-agent "$root/usr/local/bin/host-m-agent"
install -m 0755 "$script_dir/host-m-agent-logrotate" \
  "$root/usr/local/libexec/host-m-agent-logrotate"
sed "s/@HOST_M_AGENT_PACKAGE_VERSION@/$VERSION/g" "$script_dir/uninstall.sh" \
  >"$root/usr/local/share/host-m-agent/uninstall.sh"
chmod 0755 "$root/usr/local/share/host-m-agent/uninstall.sh"
install -m 0644 "$script_dir/newsyslog.conf" \
  "$root/usr/local/share/host-m-agent/newsyslog.conf"
install -m 0644 "$script_dir/org.sarmg.hostmagent.plist" \
  "$root/Library/LaunchDaemons/org.sarmg.hostmagent.plist"
install -m 0644 "$script_dir/org.sarmg.hostmagent.logrotate.plist" \
  "$root/Library/LaunchDaemons/org.sarmg.hostmagent.logrotate.plist"
sed 's#"state_dir": "/var/lib/host-m-agent"#"state_dir": "/Library/Application Support/host-m-agent"#' \
  "$agent_dir/config.example.json" \
  >"$root/usr/local/share/host-m-agent/config.example.json"
grep -F '"state_dir": "/Library/Application Support/host-m-agent"' \
  "$root/usr/local/share/host-m-agent/config.example.json" >/dev/null ||
  die "could not bind the packaged configuration to the macOS Agent state directory"
chmod 0644 "$root/usr/local/share/host-m-agent/config.example.json"

component="$packages/host-m-agent-component.pkg"
pkgbuild --root "$root" --scripts "$package_scripts" --ownership recommended \
  --identifier org.sarmg.hostmagent --version "$VERSION" --install-location / \
  "$component"
