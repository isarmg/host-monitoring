...
make_case() {
  ...
  case_state="$case_root/Library/Application Support/host-m-agent"
  case_log="$case_root/var/log/host-m-agent.log"
  case_share="$case_root/usr/local/share/host-m-agent"
  case_ownership="$case_root/var/db/host-m-agent"
  ...
  sed \
    ...
    -e "s|/Library/Application Support/host-m-agent|$case_state|g" \
    -e "s|/var/log/host-m-agent.log|$case_log|g" \
    -e "s|/usr/local/share/host-m-agent|$case_share|g" \
    -e "s|/var/db/host-m-agent|$case_ownership|g" \
...
case "$path" in
  "$CASE_ROOT/var/db/host-m-agent") metadata="${STAT_OWNERSHIP_DIR:-0:0:0:700}" ;;
...
#!/bin/sh
if [ "$#" -eq 1 ] && [ "$1" = "$CASE_ROOT/var/db/host-m-agent" ] &&
  ...
  --pkgs=*)
    ...
    case "${PKGUTIL_RECEIPT_STATE:-present}" in
      ...
      error)
        ...
        exit 74
        ;;
      unexpected) printf 'org.sarmg.hostmagent.helper\n' ;;
      *) exit 64 ;;
    esac
    ;;
  --forget) exit 0 ;;
...
case "$operation" in
  -list)
    ...
    case "$record_type:$field" in
      /Users:)
        ...
        [ "${USER_STATE:-present}" != present ] || printf '_hostmagent\n'
        ;;
      /Groups:)
        [ "${GROUP_STATE:-present}" != unknown ] || exit 72
        [ "${GROUP_STATE:-present}" != present ] || printf '_hostmagent\n'
        ;;
  ...
  -read)
    ...
    case "$record:$field" in
      /Users/_hostmagent:RealName) printf 'RealName: host-m-agent\n' ;;
      /Users/_hostmagent:UniqueID) printf 'UniqueID: 450\n' ;;
      /Users/_hostmagent:PrimaryGroupID) printf 'PrimaryGroupID: 450\n' ;;
      /Users/_hostmagent:UserShell) printf 'UserShell: /usr/bin/false\n' ;;
      /Users/_hostmagent:NFSHomeDirectory) printf 'NFSHomeDirectory: /var/empty\n' ;;
      /Groups/_hostmagent:RealName) printf 'RealName: host-m-agent\n' ;;
      /Groups/_hostmagent:PrimaryGroupID) printf 'PrimaryGroupID: 450\n' ;;
      /Groups/_hostmagent:GeneratedUID)
        printf 'GeneratedUID: 01234567-89AB-CDEF-0123-456789ABCDEF\n'
        ;;
      /Groups/_hostmagent:)
        printf 'RealName: host-m-agent\n'
  ...
  -delete)
    ...
    case "$record" in
      /Users/_hostmagent) [ "${FAIL_DELETE_USER:-0}" -eq 0 ] ;;
      /Groups/_hostmagent) [ "${FAIL_DELETE_GROUP:-0}" -eq 0 ] ;;
      *) exit 64 ;;
    esac
    ;;
  *) exit 64 ;;
...
run_case() {
  ...
  env CASE_ROOT="$run_root" "$@" \
    "$run_root/usr/local/share/host-m-agent/uninstall.sh" --purge --yes \
...
assert_incomplete() {
  ...
  [ -e "$incomplete_root/usr/local/share/host-m-agent/uninstall.sh" ] ||
    fail "incomplete purge removed its maintenance helper: $incomplete_root"
  if grep -Fx -- '--forget org.sarmg.hostmagent' "$incomplete_root/pkgutil-calls" >/dev/null 2>&1; then
...
for preserved_path in \
  ...
  "$case_root/usr/local/libexec/host-m-agent-logrotate" \
  "$case_root/usr/local/libexec/host-m-agent" \
  "$case_root/Library/Application Support/host-m-agent/state-sentinel" \
...
if grep -Fx -- '--forget org.sarmg.hostmagent' "$case_root/pkgutil-calls" >/dev/null 2>&1; then
  fail 'launchd inspection failure forgot the package receipt'
fi
grep -F 'Could not inspect system/org.sarmg.hostmagent.logrotate (launchctl status 79)' \
...
grep -Fx '/Users/_hostmagent' "$case_root/dscl-deletes" >/dev/null ||
  fail 'valid proof did not delete the owned user'
grep -Fx '/Groups/_hostmagent' "$case_root/dscl-deletes" >/dev/null ||
  fail 'valid proof did not delete the owned group'
grep -Fx -- '--forget org.sarmg.hostmagent' "$case_root/pkgutil-calls" >/dev/null ||
  fail 'complete purge did not forget the receipt'
[ ! -e "$case_root/usr/local/share/host-m-agent/uninstall.sh" ] ||
  fail 'complete purge retained the helper'
[ ! -e "$case_root/var/db/host-m-agent" ] ||
...
[ -d "$case_root/var/db/host-m-agent" ] || fail 'missing proof removed bookkeeping during incomplete purge'

case_root="$test_root/invalid-directory-acl"
make_case "$case_root"
write_marker "$case_root" 1 1
cp "$case_root/var/db/host-m-agent/account-ownership" "$case_root/marker.before"
...
cmp "$case_root/marker.before" "$case_root/var/db/host-m-agent/account-ownership" >/dev/null ||
...
[ -e "$case_root/var/db/host-m-agent/account-ownership" ] ||
...
if grep -Fx '/Groups/_hostmagent' "$case_root/dscl-deletes" >/dev/null 2>&1; then
  fail 'group was deleted after its owned user could not be removed'
fi
grep -Fx 'user_created=1' "$case_root/var/db/host-m-agent/account-ownership" >/dev/null ||
...
assert_incomplete "$case_root"
grep -Fx 'user_created=0' "$case_root/var/db/host-m-agent/account-ownership" >/dev/null ||
  fail 'partial purge did not record the completed user deletion'
grep -Fx 'group_created=1' "$case_root/var/db/host-m-agent/account-ownership" >/dev/null ||
...
make_case "$case_root"
write_marker "$case_root" 0 0
: >"$case_root/var/db/host-m-agent/.account-ownership.stale"
run_case "$case_root"
assert_incomplete "$case_root"
[ -e "$case_root/var/db/host-m-agent/account-ownership" ] ||
  fail 'unexpected bookkeeping entry caused the completed marker to be lost'
[ -e "$case_root/var/db/host-m-agent/.account-ownership.stale" ] ||
...
grep -Fx 'group_created=0' "$case_root/var/db/host-m-agent/account-ownership" >/dev/null ||
