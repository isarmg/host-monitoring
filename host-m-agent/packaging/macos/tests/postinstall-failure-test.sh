...
make_case() {
  ...
  case_state="$case_dir/Library/Application Support/host-m-agent"
  case_ownership="$case_dir/var/db/host-m-agent"
  case_log="$case_dir/var/log/host-m-agent.log"
  case_agent_binary="$case_dir/usr/local/libexec/host-m-agent"
  case_agent_command="$case_dir/usr/local/bin/host-m-agent"
  case_logrotate_helper="$case_dir/usr/local/libexec/host-m-agent-logrotate"
  case_agent_plist="$case_dir/Library/LaunchDaemons/org.sarmg.hostmagent.plist"
  case_logrotate_plist="$case_dir/Library/LaunchDaemons/org.sarmg.hostmagent.logrotate.plist"
  case_share="$case_dir/usr/local/share/host-m-agent"
  ...
  {
    printf '{\n'
    printf '  "application_version": "@HOST_M_AGENT_PACKAGE_VERSION@",\n'
...
#!/bin/sh
[ "${1:-}" = --version ] || exit 64
printf 'host-m-agent @HOST_M_AGENT_PACKAGE_VERSION@\n'
EOF
...
<?xml version="1.0"?><plist version="1.0"><dict><key>Label</key><string>org.sarmg.hostmagent</string></dict></plist>
EOF
  cat >"$case_logrotate_plist" <<'EOF'
<?xml version="1.0"?><plist version="1.0"><dict><key>Label</key><string>org.sarmg.hostmagent.logrotate</string></dict></plist>
EOF
  printf '/var/log/host-m-agent.log _hostmagent:_hostmagent 600 7 10240 * JN\n' \
...
EOF
  ...
  ln -s ../libexec/host-m-agent "$case_agent_command"
...
mkdir -p "$destination"
if [ "$destination" = "$CASE_ROOT/Library/Application Support/host-m-agent" ]; then
...
case "$path" in
  ...
  "$CASE_ROOT/usr/local/share")
    ...
    ;;
  "$CASE_ROOT/usr/local/share/host-m-agent")
  ...
  "$CASE_ROOT/Library/Application Support")
    ...
    ;;
  "$CASE_ROOT/usr/local/libexec/host-m-agent")
    metadata="${STAT_AGENT_BINARY:-0:0:755}"
    ;;
  "$CASE_ROOT/usr/local/bin/host-m-agent")
    metadata="${STAT_AGENT_COMMAND:-0:0:755}"
    ;;
  "$CASE_ROOT/usr/local/libexec/host-m-agent-logrotate")
    metadata="${STAT_LOGROTATE_HELPER:-0:0:755}"
    ;;
  "$CASE_ROOT/Library/LaunchDaemons/org.sarmg.hostmagent.plist")
    metadata="${STAT_AGENT_PLIST:-0:0:644}"
    ;;
  "$CASE_ROOT/Library/LaunchDaemons/org.sarmg.hostmagent.logrotate.plist")
    metadata="${STAT_LOGROTATE_PLIST:-0:0:644}"
    ;;
  "$CASE_ROOT/usr/local/share/host-m-agent/newsyslog.conf")
    metadata="${STAT_NEWSYSLOG_CONFIG:-0:0:644}"
    ;;
  "$CASE_ROOT/usr/local/share/host-m-agent/config.example.json")
    metadata="${STAT_PACKAGE_CONFIG:-0:0:644}"
    ;;
  "$CASE_ROOT/usr/local/share/host-m-agent/uninstall.sh")
    metadata="${STAT_UNINSTALL_HELPER:-0:0:755}"
    ;;
  "$CASE_ROOT/var/db/host-m-agent")
    metadata="${STAT_OWNERSHIP_DIR:-0:0:700}"
    ;;
  "$CASE_ROOT/var/db/host-m-agent/account-ownership"|\
  "$CASE_ROOT/var/db/host-m-agent"/.account-ownership.*)
    metadata="${STAT_OWNERSHIP_MARKER:-0:0:600}"
    ;;
  "$CASE_ROOT/Library/Application Support/host-m-agent")
    ...
    ;;
  "$CASE_ROOT/Library/Application Support/host-m-agent/config.json")
  ...
  "$CASE_ROOT/var/log")
    ...
    ;;
  "$CASE_ROOT/var/log/host-m-agent.log")
...
case "$path" in
  ...
  "$CASE_ROOT/Library/Application Support")
    ...
    permissions=drwxr-xr-x
    ;;
  "$CASE_ROOT/usr/local/libexec/host-m-agent") acl_key=agent-binary ;;
  "$CASE_ROOT/usr/local/bin/host-m-agent")
    acl_key=agent-command
    permissions=lrwxr-xr-x
    ;;
  "$CASE_ROOT/usr/local/libexec/host-m-agent-logrotate") acl_key=logrotate-helper ;;
  "$CASE_ROOT/Library/LaunchDaemons/org.sarmg.hostmagent.plist") acl_key=agent-plist ;;
  "$CASE_ROOT/Library/LaunchDaemons/org.sarmg.hostmagent.logrotate.plist")
    acl_key=logrotate-plist
    ;;
  "$CASE_ROOT/usr/local/share/host-m-agent/newsyslog.conf") acl_key=newsyslog-config ;;
  "$CASE_ROOT/usr/local/share/host-m-agent/config.example.json") acl_key=package-config ;;
  "$CASE_ROOT/usr/local/share/host-m-agent/uninstall.sh") acl_key=uninstall-helper ;;
  "$CASE_ROOT/var/log")
    acl_key=log-dir
    permissions=drwxr-xr-x
    ;;
...
case "$2" in
  "$CASE_ROOT/var/db/host-m-agent") acl_key=ownership-dir ;;
  "$CASE_ROOT/var/db/host-m-agent/account-ownership") acl_key=ownership-marker ;;
  "$CASE_ROOT/var/db/host-m-agent"/.account-ownership.*) acl_key=ownership-temp ;;
  "$CASE_ROOT/Library/Application Support/host-m-agent") acl_key=state-dir ;;
  "$CASE_ROOT/Library/Application Support/host-m-agent/config.json") acl_key=retained-config ;;
  "$CASE_ROOT/Library/Application Support/host-m-agent"/.config.json.*) acl_key=config-temp ;;
  "$CASE_ROOT/var/log/host-m-agent.log") acl_key=log-file ;;
  *) exit 65 ;;
esac
...
case "$owner" in
  ...
  _hostmagent:_hostmagent|450:450) numeric_owner=450:450 ;;
  *:*) numeric_owner="$owner" ;;
  *) exit 64 ;;
esac
for path do
...
[ "$#" -eq 3 ] && { [ "$1" = -u ] || [ "$1" = -U ]; } &&
  [ "$3" = . ] || exit 64
process_marker="$LAUNCH_STATE/process-${1#-}.org.sarmg.hostmagent"
...
[ "$#" -eq 4 ] && [ "$1" = -KILL ] &&
  { [ "$2" = -u ] || [ "$2" = -U ]; } && [ "$4" = . ] || exit 64
process_marker="$LAUNCH_STATE/process-${2#-}.org.sarmg.hostmagent"
...
if [ "${PKGUTIL_INSTALLED:-0}" -eq 1 ]; then
  printf 'package-id: org.sarmg.hostmagent\n'
  printf 'version: @HOST_M_AGENT_PACKAGE_VERSION@\n'
  exit 0
fi
exit 1
EOF
...
case "${1:-}" in
  ...
  bootstrap)
    ...
    if [ "$label" = org.sarmg.hostmagent ]; then
  ...
  bootout)
    ...
    if [ "$label" = org.sarmg.hostmagent ] &&
      [ "${BOOTOUT_LEAVES_AGENT_PROCESS:-0}" -ne 1 ]; then
      rm -f "$LAUNCH_STATE/process-u.$label" "$LAUNCH_STATE/process-U.$label"
    fi
    if [ "$label" = org.sarmg.hostmagent.logrotate ] &&
      [ "${HELPER_RESTORES_AGENT_ON_EXIT:-0}" -eq 1 ]; then
      : >"$LAUNCH_STATE/loaded.org.sarmg.hostmagent"
      : >"$LAUNCH_STATE/process-u.org.sarmg.hostmagent"
      : >"$LAUNCH_STATE/process-U.org.sarmg.hostmagent"
    fi
    if [ "$label" = org.sarmg.hostmagent ]; then
    ...
    if [ "$label" = org.sarmg.hostmagent ] &&
      ...
      config="$CASE_ROOT/Library/Application Support/host-m-agent/config.json"
...
write_ownership_marker_ids() {
  ...
  marker_group_gid="$4"
  mkdir -p "$marker_case/var/db/host-m-agent"
  {
    printf 'format=@HOST_M_AGENT_PACKAGE_VERSION@\n'
    printf 'user_created=1\n'
    printf 'user_uid=%s\n' "$marker_uid"
    printf 'user_primary_gid=%s\n' "$marker_primary_gid"
    printf 'group_created=1\n'
    printf 'group_gid=%s\n' "$marker_group_gid"
  } >"$marker_case/var/db/host-m-agent/account-ownership"
}
...
assert_payload_rejected_without_bootout() {
  ...
  payload_case_name="$2"
  shift 2
  : >"$payload_case/launch/loaded.org.sarmg.hostmagent"
  : >"$payload_case/launch/loaded.org.sarmg.hostmagent.logrotate"
  ...
  [ -e "$payload_case/launch/loaded.org.sarmg.hostmagent" ] ||
    fail "$payload_case_name stopped the previously loaded Agent"
  [ -e "$payload_case/launch/loaded.org.sarmg.hostmagent.logrotate" ] ||
  ...
  [ ! -d "$payload_case/dscl/Groups/_hostmagent" ] ||
    fail "$payload_case_name created a service group before rejecting the payload"
  [ ! -d "$payload_case/dscl/Users/_hostmagent" ] ||
...
while [ -n "$payload_failure" ]; do
  ...
  case "$payload_failure" in
    missing-agent)
      rm "$case_dir/usr/local/libexec/host-m-agent"
      next_payload_failure=redirected-agent
      ;;
    redirected-agent)
      rm "$case_dir/usr/local/libexec/host-m-agent"
      ln -s /bin/true "$case_dir/usr/local/libexec/host-m-agent"
    ...
    agent-mode)
      ...
      ;;
    agent-version)
      sed 's/host-m-agent @HOST_M_AGENT_PACKAGE_VERSION@/host-m-agent 0.0.0/' \
        "$case_dir/usr/local/libexec/host-m-agent" >"$case_dir/agent.invalid"
      mv "$case_dir/agent.invalid" "$case_dir/usr/local/libexec/host-m-agent"
      chmod 0755 "$case_dir/usr/local/libexec/host-m-agent"
      next_payload_failure=missing-helper
      ;;
    missing-helper)
      rm "$case_dir/usr/local/libexec/host-m-agent-logrotate"
      next_payload_failure=helper-syntax
      ;;
    helper-syntax)
      printf '#!/bin/sh\nif then\n' >"$case_dir/usr/local/libexec/host-m-agent-logrotate"
      chmod 0755 "$case_dir/usr/local/libexec/host-m-agent-logrotate"
    ...
    helper-mode)
      ...
      ;;
    missing-agent-plist)
      rm "$case_dir/Library/LaunchDaemons/org.sarmg.hostmagent.plist"
      next_payload_failure=invalid-agent-plist
      ;;
    invalid-agent-plist)
      printf 'not a plist\n' >"$case_dir/Library/LaunchDaemons/org.sarmg.hostmagent.plist"
    ...
    missing-logrotate-plist)
      rm "$case_dir/Library/LaunchDaemons/org.sarmg.hostmagent.logrotate.plist"
      next_payload_failure=invalid-logrotate-plist
      ;;
    invalid-logrotate-plist)
      printf 'not a plist\n' >"$case_dir/Library/LaunchDaemons/org.sarmg.hostmagent.logrotate.plist"
      next_payload_failure=missing-newsyslog
      ;;
    missing-newsyslog)
      rm "$case_dir/usr/local/share/host-m-agent/newsyslog.conf"
      next_payload_failure=newsyslog-symlink
      ;;
    newsyslog-symlink)
      rm "$case_dir/usr/local/share/host-m-agent/newsyslog.conf"
      ln -s /etc/hosts "$case_dir/usr/local/share/host-m-agent/newsyslog.conf"
    ...
    missing-package-config)
      rm "$case_dir/usr/local/share/host-m-agent/config.example.json"
      next_payload_failure=package-config-symlink
      ;;
    package-config-symlink)
      rm "$case_dir/usr/local/share/host-m-agent/config.example.json"
      ln -s /etc/hosts "$case_dir/usr/local/share/host-m-agent/config.example.json"
    ...
    package-config-version)
      sed 's/@HOST_M_AGENT_PACKAGE_VERSION@/0.0.0/' \
        "$case_dir/usr/local/share/host-m-agent/config.example.json" \
    ...
    missing-uninstall-helper)
      rm "$case_dir/usr/local/share/host-m-agent/uninstall.sh"
      next_payload_failure=uninstall-helper-symlink
      ;;
    uninstall-helper-symlink)
      rm "$case_dir/usr/local/share/host-m-agent/uninstall.sh"
      ln -s /bin/true "$case_dir/usr/local/share/host-m-agent/uninstall.sh"
    ...
    uninstall-helper-syntax)
      printf '#!/bin/sh\nif then\n' >"$case_dir/usr/local/share/host-m-agent/uninstall.sh"
      chmod 0755 "$case_dir/usr/local/share/host-m-agent/uninstall.sh"
      next_payload_failure=command-not-link
      ;;
    command-not-link)
      rm "$case_dir/usr/local/bin/host-m-agent"
      : >"$case_dir/usr/local/bin/host-m-agent"
      next_payload_failure=command-target
      ;;
    command-target)
      rm "$case_dir/usr/local/bin/host-m-agent"
      ln -s ../libexec/not-host-m-agent "$case_dir/usr/local/bin/host-m-agent"
...
make_case "$case_dir"
rm -rf "$case_dir/usr/local/share/host-m-agent"
...
while IFS='|' read -r privileged_case privileged_environment; do
  ...
  assert_payload_rejected_without_bootout "$case_dir" "$privileged_case" \
    "$privileged_environment"
  [ ! -e "$case_dir/var/db/host-m-agent" ] ||
...
[ -f "$case_dir/usr/local/share/host-m-agent/config.example.json" ] ||
  fail 'successful install lost the root-owned package config template'
[ -f "$case_dir/Library/Application Support/host-m-agent/config.json" ] ||
  fail 'successful install did not initialize the retained config'
[ ! -e "$case_dir/Library/Application Support/host-m-agent/config.example.json" ] ||
...
assert_mock_metadata "$case_dir" 450:450:0:700 \
  "$case_dir/Library/Application Support/host-m-agent"
assert_mock_metadata "$case_dir" 450:450:0:600 \
  "$case_dir/Library/Application Support/host-m-agent/config.json"
assert_mock_metadata "$case_dir" 450:450:0:600 \
  "$case_dir/var/log/host-m-agent.log"
...
assert_mock_no_acl "$case_dir" \
  "$case_dir/Library/Application Support/host-m-agent/config.json"
assert_mock_no_acl "$case_dir" "$case_dir/var/log/host-m-agent.log"
grep -Fx "0:0 $case_dir/Library/Application Support/host-m-agent" \
  "$case_dir/chown-calls" >/dev/null ||
  fail 'successful install did not lock state as root before mutation'
grep -Fx "450:450 $case_dir/Library/Application Support/host-m-agent" \
  "$case_dir/chown-calls" >/dev/null ||
  fail 'successful install did not return state ownership to the service account'
grep -Fx 'disable system/org.sarmg.hostmagent' "$case_dir/launch/calls" >/dev/null ||
  fail 'successful install did not disable Agent autoload during mutable-state validation'
grep -Fx 'enable system/org.sarmg.hostmagent' "$case_dir/launch/calls" >/dev/null ||
...
assert_recoverable() {
  ...
  [ -d "$recovery_case/dscl/Groups/_hostmagent" ] || fail "group missing after recovery"
  [ -d "$recovery_case/dscl/Users/_hostmagent" ] || fail "user missing after recovery"
  grep -Fx 'group_created=1' "$recovery_case/var/db/host-m-agent/account-ownership" >/dev/null ||
    fail "group ownership was not committed after recovery"
  grep -Fx 'user_created=1' "$recovery_case/var/db/host-m-agent/account-ownership" >/dev/null ||
...
make_case "$case_dir"
mkdir -p "$case_dir/foreign-ownership" "$(dirname -- "$case_dir/var/db/host-m-agent")"
: >"$case_dir/foreign-ownership/sentinel"
ln -s "$case_dir/foreign-ownership" "$case_dir/var/db/host-m-agent"
...
make_case "$case_dir"
mkdir -p "$case_dir/var/db/host-m-agent"
...
ln -s "$case_dir/foreign-marker" "$case_dir/var/db/host-m-agent/account-ownership"
if run_postinstall "$case_dir" >"$case_dir/failure.log" 2>&1; then
...
rm -rf "$case_dir/Library/Application Support/host-m-agent"
ln -s "$case_dir/foreign-state" "$case_dir/Library/Application Support/host-m-agent"
if run_postinstall "$case_dir" >"$case_dir/failure.log" 2>&1; then
...
rm "$case_dir/Library/Application Support/host-m-agent/config.json"
cp "$case_dir/usr/local/share/host-m-agent/config.example.json" \
...
ln -s "$case_dir/foreign-log" "$case_dir/var/log/host-m-agent.log"
if run_postinstall "$case_dir" >"$case_dir/failure.log" 2>&1; then
...
for unsafe_case in "$test_root"/unsafe-* "$test_root"/stale-package-config; do
  [ ! -d "$unsafe_case/dscl/Groups/_hostmagent" ] ||
    fail "unsafe root case created a service group: $unsafe_case"
  [ ! -d "$unsafe_case/dscl/Users/_hostmagent" ] ||
...
while [ "$failure" -le 12 ]; do
  ...
  [ ! -d "$case_dir/dscl/Users/_hostmagent" ] ||
  ...
  if [ "$failure" -le 4 ]; then
    ...
    [ ! -e "$case_dir/var/db/host-m-agent/account-ownership" ] ||
      fail "dscl create failure $failure left an ownership claim"
  else
    [ -d "$case_dir/dscl/Groups/_hostmagent" ] ||
      fail "user creation failure $failure removed the committed group"
    grep -Fx 'user_created=0' "$case_dir/var/db/host-m-agent/account-ownership" >/dev/null ||
...
if find "$case_dir/var/db/host-m-agent" -name '.account-ownership.*' -print -quit |
  grep . >/dev/null; then
...
grep -Fx 'user_created=1' "$case_dir/var/db/host-m-agent/account-ownership" >/dev/null ||
...
while [ -n "$mutable_fault" ]; do
  ...
  elif grep -Fx "0:0 $case_dir/Library/Application Support/host-m-agent" \
...
config_path="$case_dir/Library/Application Support/host-m-agent/config.json"
...
grep -Fx 'bootout system/org.sarmg.hostmagent' "$case_dir/launch/calls" >/dev/null ||
...
[ ! -e "$case_dir/launch/process-u.org.sarmg.hostmagent" ] &&
  [ ! -e "$case_dir/launch/process-U.org.sarmg.hostmagent" ] ||
  fail 'unsafe rollback left an old service-UID process alive'
[ -e "$case_dir/launch/disabled.org.sarmg.hostmagent" ] &&
  [ -e "$case_dir/launch/disabled.org.sarmg.hostmagent.logrotate" ] ||
  fail 'unsafe rollback did not leave both labels disabled'
if grep -Fx "0:0 $case_dir/Library/Application Support/host-m-agent" \
  "$case_dir/chown-calls" >/dev/null 2>&1; then
...
[ ! -e "$case_dir/launch/loaded.org.sarmg.hostmagent" ] ||
  fail "Agent remained loaded after helper bootstrap failed"
[ ! -e "$case_dir/launch/loaded.org.sarmg.hostmagent.logrotate" ] ||
...
[ -d "$case_dir/dscl/Groups/_hostmagent" ] && [ -d "$case_dir/dscl/Users/_hostmagent" ] ||
...
assert_calls_in_order_after "$case_dir/launch/calls" \
  "bootstrap system $case_dir/Library/LaunchDaemons/org.sarmg.hostmagent.logrotate.plist" \
  'disable system/org.sarmg.hostmagent.logrotate' \
  'disable system/org.sarmg.hostmagent' \
  'print system/org.sarmg.hostmagent.logrotate' \
  'print system/org.sarmg.hostmagent' \
  'bootout system/org.sarmg.hostmagent' \
  'print system/org.sarmg.hostmagent' \
  'kill SIGKILL system/org.sarmg.hostmagent' \
  'bootout system/org.sarmg.hostmagent' \
  'print system/org.sarmg.hostmagent'
grep -Fx -- '-KILL -u 450 .' "$case_dir/pkill-calls" >/dev/null ||
...
grep -F 'Could not inspect system/org.sarmg.hostmagent (launchctl status 79)' \
  "$case_dir/failure.log" >/dev/null ||
  fail 'rollback launchd inspection failure was mistaken for an absent label'
grep -F 'host-m-agent rollback cleanup is incomplete' "$case_dir/failure.log" >/dev/null ||
...
sed 's/0.0.0/@HOST_M_AGENT_PACKAGE_VERSION@/' \
  "$case_dir/usr/local/share/host-m-agent/config.example.json" \
...
reset_fault_counters "$case_dir"
: >"$case_dir/launch/calls"
rm -f "$case_dir/launch/loaded.org.sarmg.hostmagent"
...
grep -Fx 'bootout system/org.sarmg.hostmagent.logrotate' "$case_dir/launch/calls" >/dev/null ||
