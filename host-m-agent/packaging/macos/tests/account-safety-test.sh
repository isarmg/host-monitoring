...
grep -F 's/@HOST_M_AGENT_PACKAGE_VERSION@/$VERSION/g' "$build_script" >/dev/null || {
  echo "package build does not bind lifecycle scripts to VERSION" >&2
  exit 1
}
grep -F '"state_dir": "/Library/Application Support/host-m-agent"' "$build_script" >/dev/null || {
  echo "package build does not bind the config template to the macOS state directory" >&2
  exit 1
}
grep -F '$root/usr/local/share/host-m-agent/config.example.json' "$build_script" >/dev/null || {
  echo "package build does not stage config.example.json in root-owned shared data" >&2
  exit 1
}
if grep -F '$root/Library/Application Support/host-m-agent' "$build_script" >/dev/null; then
...
dscl() {
  ...
  case \"\$operation\" in
    ...
    '. -read /Groups/_hostmagent')
      ...
      ;;
    '. -read /Groups/_hostmagent GeneratedUID')
    ...
    '. -read /Groups/admin')
      ...
      case \"\$mode\" in
        ...
        name_parent) printf 'GroupMembership: _hostmagent\\n' ;;
...
group_name='host-m-agent'
group_status=0
group_is_in_use _hostmagent 450 || group_status=\"\$?\"
[ \"\$group_name\" = 'host-m-agent' ] || exit 76
exit \"\$group_status\""
  set +e
...
agent_bootout_line="$(awk '/^[[:space:]]*launchctl bootout system\/org\.sarmg\.hostmagent([[:space:]]|$)/ { line=NR } END { print line }' "$postinstall")"
...
launchd_disable_line="$(awk '/^launchctl disable system\/org\.sarmg\.hostmagent$/ { print NR; exit }' "$postinstall")"
launchd_enable_line="$(awk '/^launchctl enable system\/org\.sarmg\.hostmagent$/ { line=NR } END { print line }' "$postinstall")"
