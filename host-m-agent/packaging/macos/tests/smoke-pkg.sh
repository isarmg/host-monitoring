...
if [[ ${1:-} != --allow-system-changes ]]; then
  ...
  echo "This test installs and purges host-m-agent on the current system." >&2
  exit 2
fi
shift
...
if [[ -z $package ]]; then
  ...
  while IFS= read -r -d '' candidate; do
    ...
    -name 'host-m-agent-*.pkg' -print0)
  if (( ${#packages[@]} != 1 )); then
    echo "expected exactly one host-m-agent pkg in $repository_root/dist; found ${#packages[@]}" >&2
    printf '  %s\n' "${packages[@]}" >&2
    exit 1
  fi
  package=${packages[0]}
...
assert_command_link() {
  [[ -L /usr/local/bin/host-m-agent ]]
  [[ $(readlink /usr/local/bin/host-m-agent) == ../libexec/host-m-agent ]]
  assert_trusted_path 0:0:0:755 /usr/local/bin/host-m-agent
}
...
assert_install_trust() {
  ...
  assert_trusted_directory 0:0:0:755 /usr/local/share/host-m-agent
  ...
  assert_trusted_regular_file 0:0:0:755 /usr/local/libexec/host-m-agent
  assert_trusted_regular_file 0:0:0:755 /usr/local/libexec/host-m-agent-logrotate
  assert_trusted_regular_file 0:0:0:755 /usr/local/share/host-m-agent/uninstall.sh
  assert_trusted_regular_file 0:0:0:644 /usr/local/share/host-m-agent/newsyslog.conf
  assert_trusted_regular_file 0:0:0:644 \
    /usr/local/share/host-m-agent/config.example.json
  assert_trusted_regular_file 0:0:0:644 /Library/LaunchDaemons/org.sarmg.hostmagent.plist
  assert_trusted_regular_file 0:0:0:644 /Library/LaunchDaemons/org.sarmg.hostmagent.logrotate.plist
  assert_command_link
}
...
assert_runtime_state_trust() {
  ...
  service_uid=$(id -u _hostmagent)
  service_gid=$(id -g _hostmagent)
  assert_trusted_directory "$service_uid:$service_gid:0:700" \
    '/Library/Application Support/host-m-agent'
  assert_trusted_regular_file "$service_uid:$service_gid:0:600" \
    '/Library/Application Support/host-m-agent/config.json'
  assert_trusted_regular_file "$service_uid:$service_gid:0:600" \
    /var/log/host-m-agent.log
  sudo test ! -e '/Library/Application Support/host-m-agent/config.example.json' &&
    sudo test ! -L '/Library/Application Support/host-m-agent/config.example.json' || {
    echo 'package config template leaked into service-writable state' >&2
    exit 1
  }
  if [[ $expect_package_template == 1 ]]; then
    grep -F '"state_dir": "/Library/Application Support/host-m-agent"' \
      /usr/local/share/host-m-agent/config.example.json >/dev/null || {
...
assert_ownership_proof() {
  assert_trusted_directory 0:0:0:700 /var/db/host-m-agent
  assert_trusted_regular_file 0:0:0:600 /var/db/host-m-agent/account-ownership
}
...
if [[ ${GITHUB_ACTIONS:-} == true && ${RUNNER_OS:-} == macOS ]]; then
  ...
  if (( unsafe_host_path == 1 )); then
    for package_path in \
      /usr/local/libexec/host-m-agent \
      /usr/local/libexec/host-m-agent-logrotate \
      /usr/local/bin/host-m-agent \
      /usr/local/share/host-m-agent
    do
    ...
    ! pkgutil --pkg-info org.sarmg.hostmagent >/dev/null 2>&1
  fi
  for directory in \
    /usr/local \
    /usr/local/libexec \
...
if grep -E '(^|/)Library/Application Support/host-m-agent(/|$)' \
  <<<"$package_payload" >/dev/null; then
  echo 'package payload contains service-writable Agent state' >&2
  exit 1
fi
grep -E '(^|/)usr/local/share/host-m-agent/config\.example\.json$' \
...
install_package
sudo launchctl print system/org.sarmg.hostmagent >/dev/null
assert_install_trust
assert_runtime_state_trust
assert_ownership_proof
sudo touch '/Library/Application Support/host-m-agent/release-lifecycle-marker'
...
[[ ! -e /usr/local/libexec/host-m-agent ]]
sudo test -e '/Library/Application Support/host-m-agent/release-lifecycle-marker'
[[ ! -e /usr/local/share/host-m-agent/config.example.json ]]
assert_preserved_uninstall_trust
...
assert_runtime_state_trust
assert_ownership_proof
sudo /usr/local/share/host-m-agent/uninstall.sh --purge --yes
sudo test ! -e '/Library/Application Support/host-m-agent'
sudo test ! -e /var/db/host-m-agent
! dscl . -read /Users/_hostmagent >/dev/null 2>&1
! dscl . -read /Groups/_hostmagent >/dev/null 2>&1
! pkgutil --pkg-info org.sarmg.hostmagent >/dev/null 2>&1
