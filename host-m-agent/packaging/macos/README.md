...
```sh
BINARY=/path/to/host-m-agent VERSION=0.6.0 ./host-m-agent/packaging/macos/build-pkg.sh
```
...
VERSION=0.6.0 \
OUTPUT=/path/to/host-m-agent-0.6.0.pkg \
...
```bash
./host-m-agent/packaging/macos/tests/smoke-pkg.sh --allow-system-changes dist/host-m-agent-0.6.0.pkg
```
...
Reinstalling the current 0.6.0 package leaves the old jobs running while Installer replaces only
...
A different package version is not migrated: purge 0.6.0 state before installing another version.
