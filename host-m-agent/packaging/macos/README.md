...
```sh
BINARY=/path/to/host-m-agent VERSION=0.7.0 ./host-m-agent/packaging/macos/build-pkg.sh
```
...
```sh
BINARY=/path/to/signed/host-m-agent \
VERSION=0.7.0 \
OUTPUT=/path/to/host-m-agent-0.7.0.pkg \
INSTALLER_IDENTITY='Developer ID Installer: Example Company (TEAMID)' \
./host-m-agent/packaging/macos/build-pkg.sh
```
...
```sh
./host-m-agent/packaging/macos/tests/validate-packaging.sh
```
...
```bash
./host-m-agent/packaging/macos/tests/smoke-pkg.sh --allow-system-changes dist/host-m-agent-0.7.0.pkg
```

## Install and pair

```sh
sudo installer -pkg host-m-agent-0.7.0.pkg -target /
...
```sh
sudo -u _hostmagent /usr/local/bin/host-m-agent status --output human \
  --config '/Library/Application Support/host-m-agent/config.json'
sudo -u _hostmagent /usr/local/bin/host-m-agent doctor --output human \
  --config '/Library/Application Support/host-m-agent/config.json'
sudo launchctl print system/org.sarmg.hostmagent
sudo tail -n 100 /var/log/host-m-agent.log
```
...
Reinstalling the current 0.7.0 package leaves the old jobs running while Installer replaces only
the root-owned payload. The packaged template lives at
`/usr/local/share/host-m-agent/config.example.json`; the pkg payload never extracts into the
...
A different package version is not migrated: purge 0.7.0 state before installing another version.
...
```sh
sudo /usr/local/share/host-m-agent/uninstall.sh --purge
```

Managed non-interactive removal may add `--yes`. Purge only deletes `_hostmagent` records for
