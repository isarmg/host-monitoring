...
```sh
BINARY=/path/to/host-m-agent VERSION=0.5.0 ./host-m-agent/packaging/macos/build-pkg.sh
```
...
VERSION=0.5.0 \
OUTPUT=/path/to/host-m-agent-0.5.0.pkg \
...
```bash
./host-m-agent/packaging/macos/tests/smoke-pkg.sh --allow-system-changes dist/host-m-agent-0.5.0.pkg
```

## Install and pair

```sh
sudo installer -pkg host-m-agent-0.5.0.pkg -target /
