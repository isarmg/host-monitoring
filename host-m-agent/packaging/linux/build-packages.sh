...
die() {
  echo "host-m-agent package build: $*" >&2
  exit 1
}
...
package_id=$(cargo pkgid --locked -p host-m-agent) || die "cannot resolve the Agent Cargo package"
case "$package_id" in
...
for lifecycle_script in \
  host-m-agent/packaging/linux/postinstall.sh \
  host-m-agent/packaging/linux/preremove.sh \
  host-m-agent/packaging/linux/postremove.sh \
  host-m-agent/packaging/linux/purge-local-state.sh
do
...
agent_binary=target/release/host-m-agent
...
config_version=$(sed -n 's/^  "application_version": "\([^"]*\)",$/\1/p' host-m-agent/config.example.json)
...
VERSION="$package_version" NFPM_ARCH="$package_arch" "$nfpm_bin" package \
  --config host-m-agent/packaging/nfpm.yaml --packager deb \
  --target "dist/host-m-agent_${package_version}_${package_arch}.deb"
VERSION="$package_version" NFPM_ARCH="$package_arch" "$nfpm_bin" package \
  --config host-m-agent/packaging/nfpm.yaml --packager rpm \
  --target "dist/host-m-agent-${package_version}.${rpm_arch}.rpm"
