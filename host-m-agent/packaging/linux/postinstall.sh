...
agent_binary=/usr/bin/host-m-agent
...
die() {
  echo "host-m-agent postinstall: $*" >&2
  exit 1
}
...
[ "$("$agent_binary" --version)" = "host-m-agent $package_version" ] ||
  ...
  if [ "$user_created_now" -eq 1 ] && [ "$user_creation_committed" -eq 0 ] &&
    ...
    if lookup_user_entry; then
      if rollback_user_is_exact; then
        userdel host-m-agent ||
          echo "host-m-agent postinstall: could not roll back the incomplete dedicated user" >&2
      else
        echo "host-m-agent postinstall: refusing to roll back a dedicated user whose identity changed" >&2
      fi
    else
      user_lookup_status=$?
      if [ "$user_lookup_status" -eq 2 ]; then
        echo "host-m-agent postinstall: could not enumerate users while rolling back account creation" >&2
      fi
    fi
  fi
  ...
  if [ "$group_created_now" -eq 1 ] && [ "$group_creation_committed" -eq 0 ] &&
    ...
    if lookup_group_entry; then
      ...
      if [ -z "$rollback_group_gid" ] || [ "$current_group_gid" != "$rollback_group_gid" ]; then
        echo "host-m-agent postinstall: refusing to roll back a dedicated group whose gid changed" >&2
      else
        ...
        case "$group_usage_status" in
          1)
            groupdel host-m-agent ||
              echo "host-m-agent postinstall: could not roll back the incomplete dedicated group" >&2
            ;;
          0)
            echo "host-m-agent postinstall: refusing to roll back an incomplete group that is in use" >&2
            ;;
          *)
            echo "host-m-agent postinstall: could not verify incomplete group usage; preserving it" >&2
            ;;
        esac
      fi
    else
      ...
      if [ "$group_lookup_status" -eq 2 ]; then
        echo "host-m-agent postinstall: could not enumerate groups while rolling back account creation" >&2
      fi
    fi
  fi
...
if lookup_group_entry; then
  ...
  [ "$group_marker_state" = absent ] ||
    die "package-managed host-m-agent group is missing"
  groupadd --system host-m-agent
  group_created_now=1
...
if [ "$group_created_now" -eq 1 ]; then
  ...
  die "existing host-m-agent group has no current $package_version ownership marker"
elif [ "$recorded_group_gid" != "$group_gid" ]; then
  die "package-managed host-m-agent group was replaced with a different gid"
fi
...
if lookup_user_entry; then
  ...
  [ "$user_marker_state" = absent ] ||
    die "package-managed host-m-agent user is missing"
  nologin_shell=/usr/sbin/nologin
  if [ ! -x "$nologin_shell" ]; then
    nologin_shell=/sbin/nologin
  fi
  [ -x "$nologin_shell" ] || die "neither /usr/sbin/nologin nor /sbin/nologin exists"
  useradd --system --gid host-m-agent --home-dir "$state_dir" \
    --shell "$nologin_shell" host-m-agent
  user_created_now=1
...
# existing account is accepted only when the current 0.7.0 marker binds its
...
[ "$user_gid" = "$group_gid" ] || die "host-m-agent user does not use the dedicated group"
[ "$user_home" = "$state_dir" ] || die "host-m-agent user has an unexpected home"
case "$user_shell" in
  /usr/sbin/nologin|/sbin/nologin) ;;
  *) die "host-m-agent user has an interactive or unexpected shell" ;;
esac
...
if [ "$user_created_now" -eq 1 ]; then
  ...
  die "existing host-m-agent user has no current $package_version ownership marker"
elif
  { [ "$recorded_user_uid" != "$user_uid" ] ||
    [ "$recorded_user_primary_gid" != "$user_gid" ]; }; then
  die "package-managed host-m-agent user was replaced with a different numeric identity"
fi

if [ "$state_dir_preexisting" -eq 1 ]; then
  read_path_metadata "$state_dir"
  [ "$path_uid:$path_gid:$path_mode" = "$user_uid:$user_gid:700" ] ||
    die "$state_dir is not owned by the recorded host-m-agent identity with permissions 0700"
else
  install -d -m 0700 -o host-m-agent -g host-m-agent "$state_dir"
  [ -d "$state_dir" ] && [ ! -L "$state_dir" ] ||
    die "$state_dir did not become a safe directory"
  read_path_metadata "$state_dir"
  [ "$path_uid:$path_gid:$path_mode" = "$user_uid:$user_gid:700" ] ||
    die "$state_dir was not created for the recorded host-m-agent identity with permissions 0700"
fi
...
require_current_config "$config_path"
chown root:host-m-agent "$config_dir"
chmod 0750 "$config_dir"
chown root:host-m-agent "$config_path"
chmod 0640 "$config_path"
read_path_metadata "$config_dir"
[ "$path_uid:$path_gid:$path_mode" = "0:$user_gid:750" ] ||
  die "$config_dir could not be secured for the host-m-agent group"
read_path_metadata "$config_path"
...
host-m-agent 服务已启动，但新安装尚未配对，当前不会发送经过授权的遥测。
请在本机发起浏览器配对：
...
配对后验证状态（只读，不发送或清理队列）：
  ...
  sudo -u host-m-agent host-m-agent status --output human \
    --config /etc/host-m-agent/config.json
  sudo -u host-m-agent host-m-agent doctor --output human \
    --config /etc/host-m-agent/config.json

查看服务和日志：

  systemctl status host-m-agent.service
  journalctl -u host-m-agent.service -n 100 --no-pager

EOF
else
  cat <<'EOF'

host-m-agent 文件已安装；当前环境没有运行 systemd，因此没有启用或启动后台服务。
进入正常启动的系统后执行：
...
本机存在设备访问组：$gpu_groups。如确需启用 GPU 指标，请显式执行：
  ...
  usermod -aG $gpu_groups host-m-agent
  mkdir -p /etc/systemd/system/host-m-agent.service.d
  cp /usr/share/host-m-agent/host-m-agent-gpu.conf \
     /etc/systemd/system/host-m-agent.service.d/gpu.conf
  systemctl daemon-reload && systemctl restart host-m-agent
...
请先确认设备节点的实际属组，再授予 host-m-agent 最小必要组权限并安装 GPU drop-in。
