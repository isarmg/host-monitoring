...
cleanup() {
  case "$test_root" in
    "${TMPDIR:-/tmp}"/host-m-agent-package-builder-test.*)
      rm -rf -- "$test_root"
      ;;
    *)
...
mkdir -p \
  "$fixture_root/host-m-agent/packaging/linux" \
  "$fixture_root/target/release" \
  "$mock_bin"
cp "$packaging_dir/build-packages.sh" \
  "$fixture_root/host-m-agent/packaging/linux/build-packages.sh"
cp "$packaging_dir/../nfpm.yaml" "$fixture_root/host-m-agent/packaging/nfpm.yaml"
for lifecycle_script in postinstall.sh preremove.sh postremove.sh purge-local-state.sh; do
  printf 'package_version=%s\n' "$package_version" \
    >"$fixture_root/host-m-agent/packaging/linux/$lifecycle_script"
done
cat >"$fixture_root/host-m-agent/config.example.json" <<EOF
{
...
#!/bin/sh
[ "\$*" = "pkgid --locked -p host-m-agent" ] || exit 64
printf '%s\n' 'path+file:///fixture/agent#host-m-agent@$package_version'
EOF
...
run_builder() {
  ...
  (
    ...
    sh host-m-agent/packaging/linux/build-packages.sh
  ) >"$builder_output" 2>&1
}
...
fixed_payload=$fixture_root/target/release/host-m-agent

reset_case
write_payload "$fixed_payload" 'Advanced Micro Devices X86-64' "host-m-agent $package_version"
assert_success amd64
grep -F -- "host-m-agent_${package_version}_amd64.deb" "$nfpm_log" >/dev/null ||
  fail 'amd64 DEB target was not requested'
grep -F -- "host-m-agent-${package_version}.x86_64.rpm" "$nfpm_log" >/dev/null ||
  fail 'amd64 RPM target did not use the RPM architecture name'

reset_case
write_payload "$fixed_payload" AArch64 "host-m-agent $package_version"
assert_success arm64
grep -F -- "host-m-agent-${package_version}.aarch64.rpm" "$nfpm_log" >/dev/null ||
...
reset_case
write_payload "$fixed_payload" 'Advanced Micro Devices X86-64' 'host-m-agent 0.3.3'
alternate_payload=$test_root/alternate-agent
write_payload "$alternate_payload" 'Advanced Micro Devices X86-64' "host-m-agent $package_version"
...
do
  ...
  printf 'package_version=0.3.3\n' \
    >"$fixture_root/host-m-agent/packaging/linux/$stale_lifecycle_script"
  if run_builder amd64; then
    fail "builder accepted stale $stale_lifecycle_script package_version"
  fi
  assert_output_contains \
    "host-m-agent/packaging/linux/$stale_lifecycle_script package_version does not match Cargo"
