...
test_root=$(mktemp -d "${TMPDIR:-/tmp}/host-m-agent-packaging-test.XXXXXX")
...
cleanup() {
  case "$test_root" in
    "${TMPDIR:-/tmp}"/host-m-agent-packaging-test.*)
      rm -rf -- "$test_root"
      ;;
    *)
...
rewrite_for_test_root() {
  ...
  destination_file=$2
  sed \
    -e 's#/var/lib/host-m-agent-package#__PACKAGE_STATE__#g' \
    -e 's#/var/lib/host-m-agent#__AGENT_STATE__#g' \
    -e 's#/etc/systemd/system/host-m-agent.service.d#__DROPIN_DIR__#g' \
    -e 's#/etc/host-m-agent#__CONFIG_DIR__#g' \
    -e 's#/run/systemd/system#__SYSTEMD_RUNTIME__#g' \
    -e 's#/usr/bin/host-m-agent#__TEST_AGENT_BINARY__#g' \
    -e 's#^PATH=/usr/sbin:/usr/bin:/sbin:/bin$#PATH=__TEST_BIN__:/usr/sbin:/usr/bin:/sbin:/bin#' \
    -e "s#__PACKAGE_STATE__#$test_root/var/lib/host-m-agent-package#g" \
    -e "s#__AGENT_STATE__#$test_root/var/lib/host-m-agent#g" \
    -e "s#__DROPIN_DIR__#$test_root/etc/systemd/system/host-m-agent.service.d#g" \
    -e "s#__CONFIG_DIR__#$test_root/etc/host-m-agent#g" \
    -e "s#__SYSTEMD_RUNTIME__#$test_root/run/systemd/system#g" \
    -e "s#__TEST_AGENT_BINARY__#$test_root/bin/host-m-agent#g" \
...
grep -Fx 'TimeoutStartSec=30s' "$packaging_dir/host-m-agent.service" >/dev/null ||
  fail 'systemd unit has no bounded readiness timeout'
if grep -Fx 'Type=simple' "$packaging_dir/host-m-agent.service" >/dev/null; then
...
case "$1:$2" in
  passwd:host-m-agent)
    ...
    echo "host-m-agent:x:$current_agent_uid:$current_agent_gid::TEST_AGENT_STATE:/usr/sbin/nologin" |
      sed "s#TEST_AGENT_STATE#$test_root/var/lib/host-m-agent#"
    ;;
  passwd:)
  ...
  group:host-m-agent)
    ...
    echo "host-m-agent:x:$current_agent_gid:${SUPPLEMENTARY_MEMBER:-}"
    ;;
  group:)
...
cat >"$test_root/bin/host-m-agent" <<EOF
#!/bin/sh
printf 'host-m-agent %s\n' '$package_version'
EOF
...
write_account_markers() {
  ...
  mkdir -p "$test_root/var/lib/host-m-agent-package"
  {
    printf 'format=%s\n' "$package_version"
    printf 'uid=%s\n' "$marker_uid"
...
reset_safe_reinstall_state() {
  ...
  rm -f -- \
    ...
    "$test_root/current-group-gid"
  write_account_markers
  mkdir -p "$test_root/var/lib/host-m-agent"
  write_package_config
}
...
for attacker_command in host-m-agent getent systemctl stat; do
...
assert_log_contains 'stop host-m-agent.service'
...
# positive remaining-instance count must not disable the validated 0.6.0 service.
: >"$TEST_LOG"
...
assert_absent "$test_root/etc/systemd/system/host-m-agent.service.d"
assert_exists "$test_root/user.deleted"
assert_exists "$test_root/group.deleted"
assert_log_contains 'userdel host-m-agent'
assert_log_contains 'groupdel host-m-agent'
assert_log_contains 'daemon-reload'
...
prepare_guarded_purge_state() {
  reset_safe_reinstall_state
  rm -rf -- "$test_root/etc/systemd/system/host-m-agent.service.d"
...
assert_guarded_purge_state_preserved
assert_log_contains 'show host-m-agent.service --property=LoadState --value'
...
assert_guarded_purge_state_preserved
assert_log_contains 'disable --now host-m-agent.service'
assert_log_contains 'show host-m-agent.service --property=ActiveState --value'
...
mkdir -p "$test_root/var/lib/host-m-agent-package" "$test_root/var/lib/host-m-agent"
write_account_markers
: >"$test_root/var/lib/host-m-agent/agent-token"
...
reset_safe_reinstall_state
rm -rf "$test_root/var/lib/host-m-agent-package"
...
reset_safe_reinstall_state
rm -f "$test_root/var/lib/host-m-agent-package/managed-group"
...
assert_exists "$test_root/group.deleted"
assert_absent "$test_root/var/lib/host-m-agent-package"
...
rm -rf "$test_root/var/lib/host-m-agent-package" \
  "$test_root/var/lib/host-m-agent" "$test_root/etc/host-m-agent"
write_package_config
START_ACCOUNTS_ABSENT=1 "$test_root/postinstall.sh" >"$test_root/fresh-install.log"
grep -Fx "format=$package_version" "$test_root/var/lib/host-m-agent-package/managed-user" >/dev/null ||
  fail 'fresh install did not version its user marker'
grep -Fx 'uid=998' "$test_root/var/lib/host-m-agent-package/managed-user" >/dev/null ||
  fail 'fresh install did not record the created uid'
grep -Fx 'primary_gid=998' "$test_root/var/lib/host-m-agent-package/managed-user" >/dev/null ||
  fail 'fresh install did not record the created primary gid'
grep -Fx 'gid=998' "$test_root/var/lib/host-m-agent-package/managed-group" >/dev/null ||
...
assert_absent "$test_root/var/lib/host-m-agent-package/managed-group"
assert_absent "$test_root/var/lib/host-m-agent-package/managed-user"
...
rm -rf "$test_root/var/lib/host-m-agent-package"
ln -s "$test_root/foreign-account-state" "$test_root/var/lib/host-m-agent-package"
...
rm -rf "$test_root/var/lib/host-m-agent"
ln -s "$test_root/foreign-agent-state" "$test_root/var/lib/host-m-agent"
...
reset_safe_reinstall_state
mv "$test_root/etc/host-m-agent" "$test_root/foreign-config-dir"
ln -s "$test_root/foreign-config-dir" "$test_root/etc/host-m-agent"
...
reset_safe_reinstall_state
mv "$test_root/etc/host-m-agent/config.json" "$test_root/foreign-config.json"
ln -s "$test_root/foreign-config.json" "$test_root/etc/host-m-agent/config.json"
if "$test_root/postinstall.sh" >"$test_root/symlink-config.log" 2>&1; then
  fail 'postinstall followed a symlinked config file'
fi

reset_safe_reinstall_state
sed -i "s/$package_version/0.3.1/" "$test_root/etc/host-m-agent/config.json"
...
assert_log_contains 'restart host-m-agent.service'
if grep -F 'host-m-agent 服务已启动' "$test_root/postinstall-failure.log" >/dev/null; then
...
assert_log_contains 'restart host-m-agent.service'
assert_log_contains 'is-active --quiet host-m-agent.service'
if grep -F 'host-m-agent 服务已启动' "$test_root/postinstall-inactive.log" >/dev/null; then
