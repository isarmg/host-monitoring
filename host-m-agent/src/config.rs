...
const DEFAULT_ENDPOINT: &str =
    "http://127.0.0.1:8081/api/modules/host-monitoring/host-m-agent/v1/report";

const AGENT_VERSION_OUTPUT: &str = concat!("host-m-agent ", env!("CARGO_PKG_VERSION"));
...
///
/// `host-protocol` 是 HTTP 契约边界的唯一常量来源；Agent 配置与 Server 校验都引用它。
...
pub const MAX_REPORT_INTERVAL_SECONDS: u64 = host_protocol::AGENT_REPORT_MAX_INTERVAL_SECONDS;
...
pub const MIN_REPORT_INTERVAL_SECONDS: f64 = host_protocol::AGENT_REPORT_MIN_INTERVAL_SECONDS;
...
impl AgentConfig {
    pub fn load_from_args() -> anyhow::Result<(Self, AgentCommand)> {
        ...
        let mut config_path = env::var_os("HOST_M_AGENT_CONFIG").map(PathBuf::from);
    ...
    fn apply_pair_options(&mut self) -> anyhow::Result<()> {
        ...
        if let Some(server) = self.server_override.as_deref() {
            ...
            self.endpoint = format!(
                "{}/api/modules/host-monitoring/host-m-agent/v1/report",
    ...
    fn apply_environment(&mut self) -> anyhow::Result<()> {
        if let Ok(value) = env::var("HOST_M_AGENT_ENDPOINT") {
            self.endpoint = value;
        }
        if let Ok(value) = env::var("HOST_M_AGENT_PAIRING_ENDPOINT") {
            self.pairing_endpoint = non_empty(value);
        }
        if let Ok(value) = env::var("HOST_M_AGENT_OTLP_ENDPOINT") {
            self.otlp_endpoint = non_empty(value);
        }
        if let Ok(value) = env::var("HOST_M_AGENT_OTLP_TOKEN") {
            self.otlp_token = non_empty(value);
        }
        if let Ok(value) = env::var("HOST_M_AGENT_STATE_DIR") {
            self.state_dir = PathBuf::from(value);
        }
        if let Ok(value) = env::var("HOST_M_AGENT_INTERVAL_SECONDS") {
            self.interval_seconds = value.parse().context("invalid interval")?;
        }
        if let Ok(value) = env::var("HOST_M_AGENT_SLOW_INTERVAL_SECONDS") {
            self.slow_interval_seconds = value.parse().context("invalid slow interval")?;
        }
        if let Ok(value) = env::var("HOST_M_AGENT_TLS_IDENTITY_PEM") {
            self.tls_identity_pem = Some(PathBuf::from(value));
        }
        if let Ok(value) = env::var("HOST_M_AGENT_TLS_IDENTITY_PKCS12") {
            self.tls_identity_pkcs12 = Some(PathBuf::from(value));
        }
        if let Ok(value) = env::var("HOST_M_AGENT_TLS_IDENTITY_PASSWORD") {
            self.tls_identity_password = Some(value);
        }
        if let Ok(value) = env::var("HOST_M_AGENT_TLS_CA_PEM") {
            self.tls_ca_pem = Some(PathBuf::from(value));
        }
        if let Ok(value) = env::var("HOST_M_AGENT_ALLOW_INSECURE_HTTP") {
            self.allow_insecure_http =
                parse_bool(&value).context("invalid HOST_M_AGENT_ALLOW_INSECURE_HTTP boolean")?;
        }
        Ok(())
    }
    ...
    pub fn pairing_endpoint(&self) -> String {
        self.pairing_endpoint.clone().unwrap_or_else(|| {
            if let Some(base) = self
                .endpoint
                .strip_suffix("/api/modules/host-monitoring/host-m-agent/v1/report")
            {
                return format!(
                    "{base}/api/modules/host-monitoring/host-m-agent/v1/pairing-requests"
                );
            }
            let mut url = reqwest::Url::parse(&self.endpoint)
                .expect("endpoint was validated before pairing_endpoint is used");
            url.set_path("/api/modules/host-monitoring/host-m-agent/v1/pairing-requests");
            url.set_query(None);
...
fn validate_tray_cancel_event(name: &str) -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    {
        const PREFIX: &str = "Local\\HostMAgentPairCancel-";
        let suffix = name
            .strip_prefix(PREFIX)
...
fn default_state_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    {
        ...
        PathBuf::from(base).join("host-m-agent")
    }
    #[cfg(target_os = "macos")]
    {
        PathBuf::from("/Library/Application Support/host-m-agent")
    }
    #[cfg(all(unix, not(target_os = "macos")))]
    {
        PathBuf::from("/var/lib/host-m-agent")
    }
}
...
fn default_config_path() -> PathBuf {
    ...
    #[cfg(all(unix, not(target_os = "macos")))]
    {
        PathBuf::from("/etc/host-m-agent/config.json")
    }
}
...
fn persist_private_config(path: &Path, bytes: &[u8]) -> anyhow::Result<()> {
    ...
    content.push(b'\n');
    // pair 通常由 root 执行，而长期服务使用 host-m-agent/_hostmagent。原配置若
...
fn print_help() {
    println!(
        "host-m-agent [run|once|probe|pair|doctor|status] [options]\n\
...
mod tests {
    ...
    fn package_version_output_is_exact() {
        assert_eq!(
            agent_version_output(),
            concat!("host-m-agent ", env!("CARGO_PKG_VERSION"))
        );
        #[cfg(target_os = "linux")]
    ...
    fn insecure_override_never_applies_to_browser_pairing() {
        assert!(
            validate_endpoint(
                "http://192.0.2.10/api/modules/host-monitoring/host-m-agent/v1/report",
                true
            )
            .is_ok(),
    ...
    fn remote_plaintext_pairing_requires_a_persisted_transport_policy() {
        let remote = "http://192.0.2.10/api/modules/host-monitoring/host-m-agent/v1/report";
    ...
    fn saving_pairing_config_preserves_the_original_plaintext_policy() {
        let root = std::env::temp_dir().join(format!(
            "host-m-agent-persisted-http-policy-{}",
            uuid::Uuid::new_v4()
        ));
