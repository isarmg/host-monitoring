...
async fn tray_pairing_cancel_wakes_without_terminating_the_process() {
    let (_process_controller, process_shutdown) = host_m_agent::service::shutdown_channel();
    let (tray_controller, tray_signal) = host_m_agent::service::shutdown_channel();
...
async fn tray_pairing_deadline_is_observed_at_network_select_boundaries() {
    let (_controller, process_shutdown) = host_m_agent::service::shutdown_channel();
    let outcome = tokio::time::timeout(
        Duration::from_secs(1),
...
async fn process_shutdown_remains_sticky_between_separate_waits() {
    let (controller, shutdown) = host_m_agent::service::shutdown_channel();
...
fn pairing_activation_loads_the_server_assigned_identity() {
    ...
    let mut host = host_m_agent::HostIdentity {
    ...
    fs::write(
        ...
        serde_json::to_vec(&serde_json::json!({
            ...
            "report_endpoint": "https://unionc.example/api/modules/host-monitoring/host-m-agent/v1/report",
