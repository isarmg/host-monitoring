...
pub(super) fn print_local_status(config: &AgentConfig) -> anyhow::Result<()> {
    ...
    match config.output_mode {
        ...
        OutputMode::Human => {
            println!("host-m-agent: {overall_state}");
...
pub(super) async fn run_read_only_doctor(config: &AgentConfig) -> anyhow::Result<()> {
    ...
    checks.push(DiagnosticCheck::new(
        ...
        (collector_errors != 0)
            .then_some("inspect capability details with `host-m-agent probe --output json`"),
        started,
    ));
    checks.push(DiagnosticCheck::new(
        "server_delivery",
        ...
        Some("use `host-m-agent doctor --delivery` for an explicit end-to-end test"),
        Instant::now(),
    ));
    ...
    match config.output_mode {
        ...
        OutputMode::Human => {
            println!("host-m-agent doctor: {status} (read-only)");
            for check in &checks {
