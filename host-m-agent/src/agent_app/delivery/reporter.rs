pub(super) async fn prepare_reporter(
    ...
    host: &mut host_m_agent::HostIdentity,
    ...
    loop {
        ...
        match progress {
            ...
            Ok(Some(PairingProgress::Active {
                ...
                .context(
                    "paired host credential could not be loaded; run `host-m-agent pair` again",
                )?;
            ...
            Ok(Some(PairingProgress::Denied {
                ...
                if last_authorization_notice != Some(notice) {
                    info!(
                        agent_state = "awaiting_authorization",
                        %request_id,
                        "browser authorization was denied; run `host-m-agent pair --server <url>`"
                    );
                    last_authorization_notice = Some(notice);
                }
                tokio::select! {
            ...
            Ok(Some(PairingProgress::Expired {
                ...
                if last_authorization_notice != Some(notice) {
                    info!(
                        agent_state = "awaiting_authorization",
                        %request_id,
                        "browser authorization expired; run `host-m-agent pair --server <url>`"
                    );
                    last_authorization_notice = Some(notice);
                }
                tokio::select! {
        ...
        if command != AgentCommand::Run {
            anyhow::bail!(
                "this host is not authorized; run `host-m-agent pair --server <url>` first"
            );
        }
        let delay = jitter(backoff, config.jitter_percent);
        ...
        if last_authorization_notice != Some(notice) {
            info!(
                agent_state = "awaiting_authorization",
                retry_seconds = delay.as_secs_f64(),
                "no host credential or pending pairing request; run `host-m-agent pair --server <url>`"
            );
            last_authorization_notice = Some(notice);
        }
        tokio::select! {
