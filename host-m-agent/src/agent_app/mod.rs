    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "host_m_agent=info".into()),
        )
        .try_init()
    ...
    if matches!(command, AgentCommand::Once | AgentCommand::Doctor) {
        ...
        if command == AgentCommand::Doctor {
            ...
            match config.output_mode {
                ...
                OutputMode::Human => println!(
                    "host-m-agent doctor: healthy; end-to-end delivery succeeded and the spool is drained."
                ),
            }
            return Ok(());
        }
...
async fn run_pairing(
    ...
    host: host_m_agent::HostIdentity,
    ...
    loop {
        ...
        match polled {
                    ...
                    .await?
                {
                    ...
                    PairingWait::Shutdown => {
                        if config.tray_events {
                            ...
                            match config.output_mode {
                                OutputMode::Human => println!(
                                    "Pairing remains pending. Re-run `host-m-agent pair` to continue this saved request.\n{}",
            ...
            Ok(Some(PairingProgress::Denied {
                ...
                anyhow::bail!(
                    "browser pairing request {request_id} was denied; run `host-m-agent pair` to \
            ...
            Ok(Some(PairingProgress::Expired {
                ...
                anyhow::bail!(
                    "browser pairing request {request_id} expired; run `host-m-agent pair` to \
                     create a new request ({activation_url})"
                );
            }
                ...
                if chrono::Utc::now() >= session.expires_at {
                    ...
                    anyhow::bail!(
                        "browser pairing request {} expired while the server was unreachable; run \
                         `host-m-agent pair --replace-pending-pairing` to explicitly create a fresh request",
                        session.request_id
                    );
                }
...
fn crate_validate_tray_activation_code(value: &str) -> anyhow::Result<()> {
    host_m_agent::tray_support::validate_activation_code(value).map(|_| ())
}
