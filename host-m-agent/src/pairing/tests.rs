...
mod tests {
    ...
    fn persisted_pairing_endpoints_are_revalidated_before_network_use() {
        ...
        let remote_plaintext =
            "http://192.0.2.10/api/modules/host-monitoring/host-m-agent/v1/pairing-requests";
    ...
    async fn persisted_creating_state_cannot_reuse_remote_plaintext_endpoint() {
        let state = StoredPairingState::Creating {
            ...
            pairing_endpoint:
                "http://192.0.2.10/api/modules/host-monitoring/host-m-agent/v1/pairing-requests".into(),
            report_endpoint: "http://192.0.2.10/api/modules/host-monitoring/host-m-agent/v1/report".into(),
            host: test_host(),
    ...
    async fn transient_plaintext_override_cannot_resume_durable_pairing_stages() {
        ...
        let pairing_endpoint =
            "http://127.0.0.1:1/api/modules/host-monitoring/host-m-agent/v1/pairing-requests".to_string();
        let report_endpoint =
            "http://192.0.2.10:1/api/modules/host-monitoring/host-m-agent/v1/report".to_string();
        let config = AgentConfig {
        ...
        let handle = thread::spawn(move || {
            ...
            assert!(
                std::str::from_utf8(&request[..read])
                    .unwrap()
                    .starts_with("POST /api/modules/host-monitoring/host-m-agent/v1/pairing-requests ")
            );
    ...
    async fn create_rejects_cross_origin_activation_url_before_showing_or_persisting_it() {
        ...
        let config = AgentConfig {
            endpoint: format!("{server}/api/modules/host-monitoring/host-m-agent/v1/report"),
    ...
    fn delayed_activation_server(
        ...
        let handle = thread::spawn(move || {
            ...
            assert!(
                std::str::from_utf8(&request[..read])
                    .unwrap()
                    .starts_with("POST /api/modules/host-monitoring/host-m-agent/v1/activate ")
            );
            seen_tx.send(()).unwrap();
            release_rx
                .recv_timeout(std::time::Duration::from_secs(10))
    ...
    fn non_json_success_points_to_the_server_origin_without_leaking_the_body() {
        let endpoint = "http://127.0.0.1/api/modules/host-monitoring/host-m-agent/v1/pairing-requests";
        ...
        assert!(!rendered.contains("/api/modules/host-monitoring/host-m-agent/v1/pairing-requests"));
    ...
    fn insecure_override_never_applies_to_remote_activation_pages() {
        ...
        assert!(
            resolve_activation_url(
                "http://127.0.0.1:8081/api/modules/host-monitoring/host-m-agent/v1/pairing-requests",
                "/modules/host-monitoring/activate/00000000-0000-4000-8000-000000000001",
            )
            .is_ok()
        );
        assert!(
            resolve_activation_url(
                "http://[::1]:8081/api/modules/host-monitoring/host-m-agent/v1/pairing-requests",
                "/modules/host-monitoring/activate/00000000-0000-4000-8000-000000000001",
            )
            .is_ok()
        );
    ...
    async fn live_pending_request_cannot_be_silently_moved_to_another_server() {
        ...
        let state = StoredPairingState::Pending {
            ...
            poll_interval: 5,
            pairing_endpoint:
                "https://old.example/api/modules/host-monitoring/host-m-agent/v1/pairing-requests".into(),
            report_endpoint: "https://old.example/api/modules/host-monitoring/host-m-agent/v1/report"
                .into(),
            bearer_secret: random_secret(),
            polling_secret: random_secret(),
        };
        persist_state(&config, &state).unwrap();
        config.endpoint = "https://new.example/api/modules/host-monitoring/host-m-agent/v1/report".into();
        let error = start_or_resume(&config, &test_host())
            .await
        ...
        for old_state in ["creating", "pending"] {
            ...
            let state = if old_state == "creating" {
                StoredPairingState::Creating {
                    ...
                    generation: Uuid::new_v4(),
                    pairing_endpoint:
                        "https://old.example/api/modules/host-monitoring/host-m-agent/v1/pairing-requests"
                            .into(),
                    report_endpoint:
                        "https://old.example/api/modules/host-monitoring/host-m-agent/v1/report".into(),
                    host: test_host(),
                    bearer_secret: random_secret(),
                    polling_secret: random_secret(),
                }
            ...
            assert!(matches!(
                load_state(&config).unwrap(),
                Some(StoredPairingState::Pending { pairing_endpoint, .. })
                    if pairing_endpoint == format!("{server}/api/modules/host-monitoring/host-m-agent/v1/pairing-requests")
            ));
            server_thread.join().unwrap();
            fs::remove_dir_all(directory).unwrap();
        }
    ...
    async fn delayed_old_activation_cannot_overwrite_a_replacement_generation() {
        ...
        let old_pairing_endpoint =
            format!("{old_server}/api/modules/host-monitoring/host-m-agent/v1/pairing-requests");
        let old_report_endpoint =
            format!("{old_server}/api/modules/host-monitoring/host-m-agent/v1/report");
        ...
        let mut new_config = AgentConfig {
            endpoint: format!("{new_server}/api/modules/host-monitoring/host-m-agent/v1/report"),
            pairing_endpoint: Some(format!(
                "{new_server}/api/modules/host-monitoring/host-m-agent/v1/pairing-requests"
            )),
            state_dir: directory.clone(),
            config_path: Some(old_config_path.clone()),
            replace_pending_pairing: true,
    ...
    async fn activating_journal_recovers_all_endpoint_bound_files() {
        for preexisting in [false, true] {
            ...
            let mut config = AgentConfig {
                endpoint: "https://old.example/api/modules/host-monitoring/host-m-agent/v1/report".into(),
                state_dir: directory.clone(),
            ...
            persist_state(
                ...
                &StoredPairingState::Activating {
                    ...
                    instance_id,
                    pairing_endpoint:
                        "https://new.example/api/modules/host-monitoring/host-m-agent/v1/pairing-requests"
                            .into(),
                    report_endpoint:
                        "https://new.example/api/modules/host-monitoring/host-m-agent/v1/report".into(),
            ...
            assert_eq!(
                ...
                Some("https://new.example/api/modules/host-monitoring/host-m-agent/v1/report")
            );
            assert_eq!(
            ...
            activate_reporter_snapshot(
                ...
                request_id,
                instance_id,
                "https://new.example/api/modules/host-monitoring/host-m-agent/v1/report",
            )
            .unwrap();
            assert_eq!(
                config.endpoint,
    ...
    fn replacing_current_active_state_preserves_its_endpoint_binding() {
        ...
        config.pairing_endpoint = Some(
            "https://new.example/api/modules/host-monitoring/host-m-agent/v1/pairing-requests".into(),
        );
        ...
        assert!(matches!(
            *creating,
            StoredPairingState::Creating { ref report_endpoint, .. }
                if report_endpoint == "https://new.example/api/modules/host-monitoring/host-m-agent/v1/report"
        ));
        assert_eq!(
            load_active_binding(&config).unwrap(),
            Some(ActiveBinding {
