...
fn render_configuration() -> HttpResponse {
    response_html(
        "200 OK",
        "<main id=app aria-busy=true><header><div><h1>host-m-agent</h1><p class=subtitle>Windows 本地控制与配对</p></div><span id=version class=version></span></header>\
...
fn response_html(status: &'static str, main: &str) -> HttpResponse {
    let body = format!(
        "<!doctype html><html lang=zh-CN><head><meta charset=utf-8><meta name=viewport content=\"width=device-width,initial-scale=1\"><title>host-m-agent</title><style>\
