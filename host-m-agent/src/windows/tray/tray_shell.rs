fn run_tray(open: bool) -> anyhow::Result<()> {
    let Some(_single_instance) = create_single_instance_mutex(
        "Local\\HostMAgentTray-4E473D9E-77F7-4F9C-AFE6-CC550E72F5A4",
        false,
    )?
    else {
...
fn tray_tooltip() -> String {
    match query_service_state() {
        Ok(state) => format!("host-m-agent · {}", state.label()),
        Err(_) => "host-m-agent · 服务状态不可用".to_string(),
    }
}
