#[cfg(not(windows))]
pub(crate) fn entry() {
    eprintln!("host-m-agent-tray is available only on Windows");
    std::process::exit(2);
}
...
pub(crate) fn entry() {
    if let Err(error) = windows_tray::entry() {
        windows_tray::show_error(&format!("host-m-agent tray failed:\n\n{error:#}"));
        std::process::exit(1);
    }
}
...
mod windows_tray {
    ...
    const WINDOW_CLASS_NAME: &str = "HostMAgentTrayMessageWindow";
    ...
    const CONFIG_DIRECTORY: &str = "host-m-agent";
    const CONFIG_FILE: &str = "config.json";
    const AGENT_EXE: &str = "host-m-agent.exe";
    const TRAY_EXE: &str = "host-m-agent-tray.exe";
    const PRIVILEGED_OPERATION_MUTEX: &str =
        "Global\\HostMAgentOperation-875F84C4-E2A4-4542-8DBF-C474E04826C4";
