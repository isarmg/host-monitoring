    ...
    fn serde_dtos_accept_only_the_current_wire_shape() {
        ...
        assert!(
            serde_json::from_str::<PairIpcMessage>(
                r#"{"generation":"generation","request_id":"request","activation_url":"https://server.example/activate/request","pairing_endpoint":"https://server.example/api/v2/agent/pairing-requests"}"#
            )
            .is_ok()
        );
        assert!(
            serde_json::from_str::<PairIpcMessage>(
                r#"{"generation":"generation","request_id":"request","activation_url":"https://server.example/activate/request","pairing_endpoint":"https://server.example/api/v2/agent/pairing-requests","legacy":true}"#
            )
            .is_err()
        );

        let waiting_event = format!(
            r#"{{"event":"pairing_waiting","version":"{}","request_id":"request","generation":"generation","activation_url":"https://server.example/activate/request","pairing_endpoint":"https://server.example/api/v2/agent/pairing-requests","expires_at":"2026-08-20T00:00:00Z","poll_interval":2}}"#,
            env!("CARGO_PKG_VERSION")
        );
        assert!(serde_json::from_str::<PairEvent>(&waiting_event).is_ok());
        let waiting_event_without_expiry = format!(
            r#"{{"event":"pairing_waiting","version":"{}","request_id":"request","generation":"generation","activation_url":"https://server.example/activate/request","pairing_endpoint":"https://server.example/api/v2/agent/pairing-requests","poll_interval":2}}"#,
            env!("CARGO_PKG_VERSION")
        );
        assert!(serde_json::from_str::<PairEvent>(&waiting_event_without_expiry).is_err());
        let paired_event_with_legacy_field = format!(
            r#"{{"event":"paired","version":"{}","request_id":"request","instance_id":"instance","endpoint":"https://server.example/api/v2/agent/report","legacy":true}}"#,
            env!("CARGO_PKG_VERSION")
        );
