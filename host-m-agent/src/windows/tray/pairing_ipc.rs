fn elevated_pair(server: String, callback_nonce: String) -> anyhow::Result<()> {
    ...
    let _pair_mutex = create_single_instance_mutex(PRIVILEGED_OPERATION_MUTEX, true)?
        .context("another host-m-agent pairing operation is already running")?;
...
fn elevated_service(action: ServiceAction, notify: bool) -> anyhow::Result<()> {
    ...
    let _operation = create_single_instance_mutex(PRIVILEGED_OPERATION_MUTEX, true)?
        .context("another host-m-agent pairing or service operation is already running")?;
    match action {
    ...
    if notify {
        message_box(
            match action {
                ServiceAction::Start => "host-m-agent 服务已启动。",
                ServiceAction::Stop => "host-m-agent 服务已停止；Windows 下次启动时仍会自动运行。",
            },
            "host-m-agent",
...
fn run_hidden_pair(server: &str, callback_nonce: &str) -> anyhow::Result<()> {
    ...
    let cancel_event_name = format!("Local\\HostMAgentPairCancel-{callback_nonce}");
