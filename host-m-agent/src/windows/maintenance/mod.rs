...
#[cfg(any(windows, test))]
const MAINTENANCE_DIAGNOSTIC_FORMAT: &str = "host-m-agent-maintenance-diagnostic-v1";
#[cfg(any(windows, test))]
...
#[cfg(not(windows))]
pub(crate) fn entry() {
    eprintln!("host-m-agent-maintenance is available only on Windows");
    std::process::exit(2);
}
...
pub(crate) fn entry() {
    if let Err(error) = windows_maintenance::run() {
        eprintln!("host-m-agent maintenance failed: {error:#}");
        std::process::exit(1);
    }
}
...
mod windows_maintenance {
    ...
    use anyhow::{Context, bail, ensure};
    use host_m_agent::{AgentConfig, service::WINDOWS_SERVICE_NAME};
    ...
    const DIRECTORY_NAME: &str = "host-m-agent";
    const AGENT_EXE: &str = "host-m-agent.exe";
    const CONFIG_FILE: &str = "config.json";
    const JOURNAL_DIRECTORY: &str =
        concat!("host-m-agent.install-journal-", env!("CARGO_PKG_VERSION"));
    const UNINSTALL_JOURNAL_DIRECTORY: &str =
        concat!("host-m-agent.uninstall-journal-", env!("CARGO_PKG_VERSION"));
    const PURGE_DIRECTORY: &str =
        concat!("host-m-agent.purge-quarantine-", env!("CARGO_PKG_VERSION"));
    const DIAGNOSTIC_FILE: &str = concat!(
        "host-m-agent.maintenance-diagnostic-",
        env!("CARGO_PKG_VERSION"),
        ".txt"
    );
    const SNAPSHOT_FORMAT: u32 = 2;
    const STATE_MARKER: &str = concat!(".host-m-agent-managed-", env!("CARGO_PKG_VERSION"));
    const STATE_MARKER_CONTENT: &str = concat!(
        "host-m-agent-windows-state-",
        env!("CARGO_PKG_VERSION"),
        "\r\n"
    );
...
mod maintenance_diagnostic_tests {
    ...
    fn diagnostic_payload_is_utf8_versioned_bounded_and_keeps_the_error_chain() {
        ...
        assert!(text.contains("format=host-m-agent-maintenance-diagnostic-v1\n"));
